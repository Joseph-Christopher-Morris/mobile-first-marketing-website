...
export function StickyConversionBar() {
  ...
  const getCTAText = () => {
    if (pathname?.includes('/photography')) return 'Book Your Shoot';
    if (pathname?.includes('/ad-campaigns') || pathname?.includes('/marketing')) return 'Get Better Results';
    if (pathname?.includes('/hosting')) return 'Upgrade My Site';
    if (pathname?.includes('/pricing')) return 'See What Fits You';
    if (pathname?.includes('/about')) return 'Work With Me';
    if (pathname?.includes('/blog')) return 'Start Your Own Success Story';
    if (pathname?.includes('/contact')) return 'Send Message';
    return "Let's Improve Your Marketing";
  };
