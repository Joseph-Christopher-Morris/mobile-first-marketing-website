...
class SSLCertificateSetup {
  ...
  async findExistingCertificate() {
    console.log(`🔍 Checking for existing SSL certificate for ${this.customDomain}...`);
    ...
    try {
      ...
      const existingCert = result.CertificateSummaryList?.find(cert => 
  ...
  async requestCertificate() {
    ...
    try {
      ...
      const result = await this.acmClient.send(new RequestCertificateCommand({
      ...
      if (error.name === 'LimitExceededException') {
        ...
        console.error('   Consider deleting unused certificates or contact AWS support.');
      }
  ...
  async getDNSValidationRecords() {
    ...
    try {
      ...
      if (!certificate.DomainValidationOptions) {
        throw new Error('DNS validation records not yet available. Please wait and try again.');
      }
      
      this.validationRecords = certificate.DomainValidationOptions.map(option => ({
  ...
  async attemptAutomaticValidation() {
    console.log('🔍 Checking for Route 53 hosted zone for automatic validation...');
    ...
    try {
      ...
      const matchingZone = hostedZones.HostedZones?.find(zone => {
        ...
        return this.customDomain === zoneName || this.customDomain.endsWith(`.${zoneName}`);
      });
      
      if (!matchingZone) {
        console.log('ℹ️  No matching Route 53 hosted zone found. Manual DNS validation required.');
        return false;
      }
   ...
   */
  async waitForCertificateValidation(maxWaitTime = 1800000) { // 30 minutes max
  ...
  async updateCloudFrontDistribution() {
    ...
    try {
      ...
      console.log(`   Distribution ID: ${this.distributionId}`);
      console.log(`   Custom domains: ${updatedConfig.Aliases.Items.join(', ')}`);
      console.log(`   SSL Certificate: ${this.certificateArn}`);
      console.log(`   TLS Version: ${updatedConfig.ViewerCertificate.MinimumProtocolVersion}`);
  ...
  generateManualInstructions() {
    ...
    console.log('\n📝 Instructions:');
    console.log('1. Log in to your DNS provider (e.g., GoDaddy, Namecheap, Route 53)');
    ...
    console.log('5. Run this script again to update CloudFront with the certificate');
    
    console.log('\n🔍 To check validation status:');
    console.log(`aws acm describe-certificate --certificate-arn ${this.certificateArn} --region us-east-1`);
  }
    ...
    try {
      ...
      if (!existingCert) {
        ...
        if (autoValidated) {
          ...
          return { status: 'pending_validation', certificateArn: this.certificateArn };
        }
      ...
      return { status: 'completed', certificateArn: this.certificateArn, config };
      
    } catch (error) {
      console.error('\n❌ SSL certificate setup failed:', error.message);
      console.error('\n🔧 Troubleshooting tips:');
      console.error('1. Verify AWS credentials have ACM and CloudFront permissions');
      console.error('2. Ensure CUSTOM_DOMAIN environment variable is set correctly');
      console.error('3. Check that CloudFront distribution exists and is accessible');
