...
class MimeTypeVerifier {
    ...
    async checkCloudfrontResponses() {
        ...
        for (const file of imageFiles) {
            try {
                ...
                console.error(`❌ Error checking CloudFront response for ${file.key}:`, error.message);
                this.results.summary.errors++;
            }
        }
    }
    ...
    async testSpecificBlogImage() {
        ...
        try {
            ...
            console.log(`   S3 MIME Type: ${s3Correct ? '✅' : '❌'} ${s3Metadata.ContentType}`);
            console.log(`   CloudFront MIME Type: ${cfCorrect ? '✅' : '❌'} ${cfHeaders['content-type']}`);
    ...
    generateReport() {
        ...
        console.log(`   Total Objects Checked: ${this.results.summary.totalChecked}`);
        console.log(`   Correct MIME Types: ${this.results.summary.correctMimeTypes}`);
        console.log(`   Incorrect MIME Types: ${this.results.summary.incorrectMimeTypes}`);
        console.log(`   Errors: ${this.results.summary.errors}`);
        console.log(`   Success Rate: ${((this.results.summary.correctMimeTypes / this.results.summary.totalChecked) * 100).toFixed(1)}%`);
    ...
    generateRecommendations() {
        ...
        const incorrectWebP = this.results.s3Objects.filter(obj => 
            obj.extension === '.webp' && !obj.correct
        );
        ...
        const missingMimeTypes = this.results.s3Objects.filter(obj => 
            !obj.actualMimeType || obj.actualMimeType === 'binary/octet-stream'
        );
    ...
    async fixIncorrectMimeTypes(dryRun = true) {
        ...
        for (const obj of incorrectObjects) {
            ...
            console.log(`${dryRun ? '🔍' : '🔧'} ${obj.key}: ${obj.actualMimeType} → ${correctMimeType}`);
...
async function main() {
    ...
    try {
        ...
        console.log('✅ S3 objects have correct Content-Type headers for WebP files');
        console.log('✅ CloudFront serves images with proper MIME types');
        console.log('✅ Browsers receive correct content-type headers');
        console.log(`✅ Blog image MIME type verification: ${blogImageResult ? 'PASSED' : 'NEEDS ATTENTION'}`);

        // Exit with appropriate code
        const hasErrors = verifier.results.summary.incorrectMimeTypes > 0 || verifier.results.summary.errors > 0;
