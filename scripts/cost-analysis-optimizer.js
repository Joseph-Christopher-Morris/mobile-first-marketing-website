...
class CostAnalysisOptimizer {
  constructor() {
    ...
    this.costReportPath = path.join(__dirname, '..', 'logs', 'cost-analysis.json');
  }
  ...
  async getCostAndUsage(startDate, endDate, granularity = 'MONTHLY') {
    try {
      ...
      console.warn('Cost Explorer API not available or insufficient permissions:', error.message);
      return null;
    }
  }
  ...
  async getS3StorageAnalysis(bucketName) {
    try {
      ...
      for (const object of objects.Contents) {
        ...
        storageClasses[storageClass] = (storageClasses[storageClass] || 0) + object.Size;
        ...
        if (object.Size > 100 * 1024 * 1024) { // > 100MB
          largeObjects.push({
            key: object.Key,
            size: object.Size,
            storageClass: object.StorageClass || 'STANDARD'
  ...
  async getCloudFrontAnalysis(distributionId) {
    try {
      ...
      const requestsMetric = await this.cloudwatch.getMetricStatistics({
      ...
      const bytesDownloadedMetric = await this.cloudwatch.getMetricStatistics({
        Namespace: 'AWS/CloudFront',
      ...
      const totalRequests = requestsMetric.Datapoints.reduce((sum, point) => sum + point.Sum, 0);
      const totalBytes = bytesDownloadedMetric.Datapoints.reduce((sum, point) => sum + point.Sum, 0);
  ...
  calculateS3Costs(storageAnalysis) {
    ...
    Object.entries(storageAnalysis.storageClasses).forEach(([storageClass, bytes]) => {
      const gb = bytes / (1024 * 1024 * 1024);
      const monthlyCost = gb * (storagePricing[storageClass] || storagePricing.STANDARD);
  ...
  calculateCloudFrontCosts(cfAnalysis) {
    ...
    const requestCost = (cfAnalysis.totalRequests30Days * requestPricing) / 10000;
    const dataTransferCost = (cfAnalysis.totalBytesDownloaded30Days / (1024 * 1024 * 1024)) * dataTransferPricing;
  ...
  generateOptimizationRecommendations(storageAnalysis, cfAnalysis, s3Costs, cfCosts) {
    const recommendations = [];
    
    // S3 Storage Optimization
    if (storageAnalysis.oldObjects.length > 0) {
      const oldObjectsSize = storageAnalysis.oldObjects.reduce((sum, obj) => sum + obj.size, 0);
      ...
      recommendations.push({
        ...
        action: 'Implement S3 lifecycle policy to transition to STANDARD_IA after 30 days'
      });
    }
    ...
    if (storageAnalysis.largeObjects.length > 0) {
      recommendations.push({
        ...
        action: 'Consider compression or breaking large files into smaller chunks'
      });
    }
    ...
    if (cfAnalysis.priceClass === 'PriceClass_All') {
      recommendations.push({
        ...
        description: 'Using all edge locations may be unnecessary for your traffic patterns',
        potentialSavings: cfCosts.total * 0.2, // Rough estimate
        action: 'Analyze traffic patterns and consider PriceClass_100 or PriceClass_200'
      });
    }
    ...
    if (cacheHitRatio < 0.8) {
      recommendations.push({
        ...
        action: 'Review cache behaviors and TTL settings to improve caching efficiency'
      });
    }
  ...
  async generateCostAnalysis() {
    ...
    if (!storageAnalysis.error) {
      ...
      console.log(`   Object Count: ${storageAnalysis.objectCount.toLocaleString()}`);
      ...
      if (storageAnalysis.oldObjects.length > 0) {
        console.log(`   Old Objects (30+ days): ${storageAnalysis.oldObjects.length}`);
      }
    } else {
    ...
    if (!cfAnalysis.error) {
      ...
      console.log(`   Requests (30 days): ${cfAnalysis.totalRequests30Days.toLocaleString()}`);
      console.log(`   Data Downloaded: ${(cfAnalysis.totalBytesDownloaded30Days / (1024 * 1024 * 1024)).toFixed(2)} GB`);
      console.log(`   Compression: ${cfAnalysis.compressionEnabled ? 'Enabled' : 'Disabled'}`);
    ...
    if (!storageAnalysis.error && !cfAnalysis.error) {
      ...
      Object.entries(s3Costs.storage).forEach(([storageClass, cost]) => {
        console.log(`     ${storageClass}: $${cost.monthlyCost.toFixed(4)} (${cost.sizeGB.toFixed(2)} GB)`);
      });
      ...
      const recommendations = this.generateOptimizationRecommendations(
        storageAnalysis, cfAnalysis, s3Costs, cfCosts
      );
      ...
      if (recommendations.length > 0) {
        ...
        recommendations.forEach((rec, index) => {
          ...
          if (rec.potentialSavings) {
            console.log(`      Potential Savings: $${rec.potentialSavings.toFixed(4)}/month`);
        ...
        if (totalPotentialSavings > 0) {
          console.log(`   💰 Total Potential Savings: $${totalPotentialSavings.toFixed(4)}/month`);
          console.log(`   📈 Potential Cost Reduction: ${((totalPotentialSavings / totalCost) * 100).toFixed(1)}%`);
        }
      } else {
