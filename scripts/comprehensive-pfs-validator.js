...
class ComprehensivePFSValidator {
    ...
    async testCipherSuitePreferences() {
        ...
        try {
            ...
            if (testResults.testPassed) {
                this.results.summary.passedTests++;
                console.log(`✅ Cipher suite preferences test: PASSED (${testResults.preferredCiphers.length} preferred ciphers)`);
            } else {
                this.results.summary.failedTests++;
            ...
            console.error(`❌ Cipher suite preferences test failed: ${error.message}`);
            this.results.summary.failedTests++;
            return { error: error.message };
        }
    }
    ...
    async testEphemeralKeyRotation() {
        ...
        try {
            ...
            if (testResults.testPassed) {
                this.results.summary.passedTests++;
                console.log(`✅ Ephemeral key rotation test: PASSED (${testResults.uniqueKeys} unique keys)`);
            } else {
                this.results.summary.failedTests++;
    ...
    async testPFSAcrossTLSVersions() {
        ...
        try {
            ...
            if (testResults.testPassed) {
                this.results.summary.passedTests++;
                console.log(`✅ TLS version PFS test: PASSED (${compliantVersions}/${tlsVersions.length} versions compliant)`);
            } else {
                this.results.summary.failedTests++;
    ...
    generateComplianceReport() {
        ...
        if (!this.results.compliance.pfsCompliant) {
            this.results.compliance.recommendations.push({
                ...
                recommendation: 'Review and update TLS configuration to improve PFS support'
            });
        }
    ...
    async runComprehensiveTests(distributionDomain = null) {
        console.log('🚀 Starting Comprehensive Perfect Forward Secrecy Validation...');
        ...
        try {
            ...
            console.log('=' .repeat(70));
            console.log(`📊 Tests Passed: ${this.results.summary.passedTests}/${this.results.summary.totalTests} (${((this.results.summary.passedTests / this.results.summary.totalTests) * 100).toFixed(1)}%)`);
            console.log(`🛡️ Security Level: ${this.results.compliance.securityLevel}`);
            console.log(`✅ PFS Compliant: ${this.results.compliance.pfsCompliant ? 'YES' : 'NO'}`);
