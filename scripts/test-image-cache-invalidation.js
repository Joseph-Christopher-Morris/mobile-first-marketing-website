...
function testImageInvalidationLogic() {
  ...
  for (const file of testFiles) {
    ...
    let shouldInvalidate = isShortCacheFile || isImageFile;
  ...
  console.log(`   Invalidation paths: ${deployment.invalidationPaths.join(', ')}`);
  ...
  deployment.invalidationPaths = [...manyImagePaths, '/index.html', '/about.html'];
  
  console.log(`   Before optimization: ${deployment.invalidationPaths.length} paths`);
...
function testSpecificImageScenarios() {
  ...
  for (const scenario of scenarios) {
    ...
    console.log(`   Files to invalidate: ${deployment.invalidationPaths.length}`);
    console.log(`   Paths: ${deployment.invalidationPaths.map(p => p.split('/').pop()).join(', ')}`);
    console.log('');
  }
...
if (require.main === module) {
  try {
    ...
    if (test1Passed && test2Passed) {
      ...
      console.log('The deployment script will properly invalidate image caches when they change.');
      process.exit(0);
    } else {
