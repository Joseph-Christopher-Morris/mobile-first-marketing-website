...
class TLSValidator {
  ...
  async testTLSVersions(domain) {
    ...
    for (const tlsVersion of tlsVersions) {
      try {
        ...
        console.log(
          `  ${supported ? '✅' : '❌'} ${tlsVersion.name}: ${supported ? 'Supported' : 'Not supported'}`
        );
      } catch (error) {
        this.results.tlsVersions[tlsVersion.name] = {
          supported: false,
    ...
    return new Promise((resolve, reject) => {
      ...
      const socket = tls.connect(options, () => {
        try {
          ...
          socket.destroy();
          reject(
            new Error(`Failed to get cipher information: ${error.message}`)
          );
        }
      });
  ...
  checkWeakConfigurations() {
    ...
    if (!this.results.tlsVersions['TLSv1.3']?.supported) {
      this.results.recommendations.push({
        ...
        severity: 'medium',
        message:
          'TLS 1.3 is not supported - consider enabling for better security',
    ...
    if (this.results.weakCiphers.length > 0) {
      this.results.recommendations.push({
        ...
        action:
          'Disable weak cipher suites and use only strong, modern ciphers',
      });
    }
  }
  ...
  validatePerfectForwardSecrecy() {
    ...
    if (this.ephemeralKeyInfo) {
      if (
        this.ephemeralKeyInfo.type === 'ECDH' ||
        this.ephemeralKeyInfo.type === 'DH'
      ) {
        hasPFS = true;
        pfsDetails.push({
          type: 'ephemeral',
    ...
    if (!hasPFS) {
      this.results.recommendations.push({
        ...
        action:
          'Configure cipher suites that support ECDHE or DHE key exchange',
      });
    }

    console.log(
      `  ${hasPFS ? '✅' : '❌'} Perfect Forward Secrecy: ${hasPFS ? 'Supported' : 'Not detected'}`
    );

    if (hasPFS && pfsDetails.length > 0) {
      console.log(
        `    Details: ${pfsDetails.map(d => d.cipher || `${d.keyType} ${d.size}-bit`).join(', ')}`
      );
    }
  }
  ...
  calculateSecurityScore() {
    ...
    const strongCiphers = this.results.cipherSuites.filter(
      c => c.strength === 'strong'
    ).length;
  ...
  generateRecommendations() {
    ...
    this.results.recommendations.push({
      ...
      action:
        'Set Security Policy to TLSv1.2_2021 or newer in CloudFront distribution settings',
    });
  }
  ...
  printSummary() {
    ...
    console.log(
      `Perfect Forward Secrecy: ${this.results.perfectForwardSecrecy ? 'Yes' : 'No'}`
    );
