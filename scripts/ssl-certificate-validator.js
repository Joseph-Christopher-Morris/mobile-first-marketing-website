...
class SSLCertificateValidator {
  ...
  async validateCertificate(hostname, port = 443) {
    console.log(`🔍 Starting SSL certificate validation for ${hostname}:${port}`);
    ...
    try {
      ...
      this.addTestResult('certificate-retrieval', 'FAILED', 
        `Failed to retrieve certificate: ${error.message}`, hostname);
      throw error;
    }
  }
  ...
  async validateCertificateValidity(certInfo, hostname) {
    ...
    if (now < validFrom) {
      this.addTestResult('validity-dates', 'FAILED', 
        `Certificate is not yet valid. Valid from: ${validFrom.toISOString()}`, hostname);
    } else if (now > validTo) {
      this.addTestResult('validity-dates', 'FAILED', 
        `Certificate has expired. Valid until: ${validTo.toISOString()}`, hostname);
    } else {
      // Check for upcoming expiration (within 30 days)
      const daysUntilExpiry = Math.floor((validTo - now) / (1000 * 60 * 60 * 24));
      
      if (daysUntilExpiry <= 30) {
        this.addTestResult('validity-dates', 'WARNING', 
          `Certificate expires in ${daysUntilExpiry} days (${validTo.toISOString()})`, hostname);
      } else {
        this.addTestResult('validity-dates', 'PASSED', 
          `Certificate is valid until ${validTo.toISOString()} (${daysUntilExpiry} days remaining)`, hostname);
      }
    }

    // Validate certificate lifetime (not too long)
    const lifetimeDays = Math.floor((validTo - validFrom) / (1000 * 60 * 60 * 24));
    if (lifetimeDays > 825) { // Current CA/Browser Forum baseline
      this.addTestResult('certificate-lifetime', 'WARNING', 
        `Certificate lifetime is ${lifetimeDays} days, which exceeds recommended 825 days`, hostname);
    } else {
      this.addTestResult('certificate-lifetime', 'PASSED', 
        `Certificate lifetime is ${lifetimeDays} days (within recommended limits)`, hostname);
    }
  }
  ...
  async validateSubjectAndSAN(certInfo, hostname) {
    ...
    if (cnMatches || sanMatches) {
      this.addTestResult('subject-san-match', 'PASSED', 
        `Hostname ${hostname} matches certificate (CN: ${subjectCN}, SAN: ${san.join(', ')})`, hostname);
    } else {
      this.addTestResult('subject-san-match', 'FAILED', 
        `Hostname ${hostname} does not match certificate (CN: ${subjectCN}, SAN: ${san.join(', ')})`, hostname);
    }
    ...
    if (missingFields.length === 0) {
      this.addTestResult('subject-completeness', 'PASSED', 
        'Certificate subject contains all recommended fields', hostname);
    } else {
      this.addTestResult('subject-completeness', 'WARNING', 
        `Certificate subject missing recommended fields: ${missingFields.join(', ')}`, hostname);
    }
  }
  ...
  async validateCertificateChain(certInfo, hostname) {
    ...
    if (!cert.issuerCertificate || cert.issuerCertificate === cert) {
      this.addTestResult('certificate-chain', 'WARNING', 
        'Certificate chain appears incomplete or self-signed', hostname);
      return;
    }
    ...
    while (currentCert && currentCert.issuerCertificate && currentCert.issuerCertificate !== currentCert) {
      ...
      if (seenCerts.has(certFingerprint)) {
        this.addTestResult('certificate-chain', 'FAILED', 
          'Certificate chain contains circular reference', hostname);
        return;
      }
      ...
      if (chainLength > 10) {
        this.addTestResult('certificate-chain', 'FAILED', 
          'Certificate chain is too long (>10 certificates)', hostname);
        return;
      }
    }

    if (chainLength >= 2 && chainLength <= 4) {
      this.addTestResult('certificate-chain', 'PASSED', 
        `Certificate chain length is appropriate (${chainLength} certificates)`, hostname);
    } else if (chainLength === 1) {
      this.addTestResult('certificate-chain', 'WARNING', 
        'Certificate chain is very short (may be missing intermediate certificates)', hostname);
    } else {
      this.addTestResult('certificate-chain', 'WARNING', 
        `Certificate chain length is unusual (${chainLength} certificates)`, hostname);
    }
  ...
  validateChainSignatureAlgorithms(cert, hostname) {
    ...
    while (currentCert && certIndex < 10) {
      ...
      if (weakAlgorithms.some(weak => sigAlg.includes(weak))) {
        issues.push(`Certificate ${certIndex} uses weak signature algorithm: ${currentCert.signatureAlgorithm}`);
      }
    ...
    if (issues.length === 0) {
      this.addTestResult('signature-algorithms', 'PASSED', 
        'All certificates in chain use strong signature algorithms', hostname);
    } else {
      this.addTestResult('signature-algorithms', 'FAILED', 
        `Weak signature algorithms detected: ${issues.join(', ')}`, hostname);
    }
  }
  ...
  async validateCertificateAuthority(certInfo, hostname) {
    ...
    if (certInfo.authorized) {
      this.addTestResult('ca-trust', 'PASSED', 
        'Certificate is trusted by system CA store', hostname);
    } else {
      const error = certInfo.authorizationError || 'Unknown authorization error';
      this.addTestResult('ca-trust', 'FAILED', 
        `Certificate is not trusted: ${error}`, hostname);
    }

    // Validate root CA information
    let rootCert = cert;
    while (rootCert.issuerCertificate && rootCert.issuerCertificate !== rootCert) {
    ...
    if (rootCert.subject.CN === rootCert.issuer.CN) {
      this.addTestResult('root-ca-validation', 'PASSED', 
        `Root CA: ${rootCert.subject.CN}`, hostname);
    } else {
      this.addTestResult('root-ca-validation', 'WARNING', 
        'Root certificate does not appear to be self-signed', hostname);
    }
  ...
  validateKeyUsage(cert, hostname) {
    ...
    const hasServerAuth = extensions.some(usage => 
      usage.includes('TLS Web Server Authentication') || 
      usage.includes('serverAuth')
    );

    if (hasServerAuth) {
      this.addTestResult('key-usage', 'PASSED', 
        'Certificate has appropriate key usage for TLS server authentication', hostname);
    } else {
      this.addTestResult('key-usage', 'WARNING', 
        'Certificate may not have appropriate key usage extensions for TLS server authentication', hostname);
    }
  }
...
async function main() {
  ...
  if (args.length === 0) {
    console.log('Usage: node ssl-certificate-validator.js <hostname> [options]');
    console.log('Options:');
    console.log('  --port <port>        Port to connect to (default: 443)');
    console.log('  --output <file>      Save results to JSON file');
    console.log('  --verbose           Enable verbose output');
    console.log('  --multiple <file>   Validate multiple domains from JSON file');
    console.log('\nExample:');
    console.log('  node ssl-certificate-validator.js example.com');
    console.log('  node ssl-certificate-validator.js example.com --port 8443 --output results.json');
    process.exit(1);
  }
