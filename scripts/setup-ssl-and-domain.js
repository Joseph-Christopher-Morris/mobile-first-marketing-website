...
class SSLAndDomainSetup {
  ...
  async setupSSLCertificate() {
    ...
    try {
      ...
      if (sslResult.status === 'completed') {
        ...
        console.log(
          '   Please complete DNS validation and run this script again'
        );
        return false;
      }
    } catch (error) {
  ...
  generateSetupSummary() {
    ...
    const summary = {
      ...
      domain: {
        ...
        httpsRedirect: true,
        dnsConfiguration: this.setupResults.domain?.dnsConfigured
          ? 'automatic'
          : 'manual',
      },
      features: {
    ...
    console.log(
      `🔐 SSL Certificate: ${this.certificateArn ? 'Configured' : 'Pending'}`
    );
    console.log(`📝 DNS Configuration: ${summary.domain.dnsConfiguration}`);
    console.log(
      `🔒 HTTPS Redirect: ${summary.features.httpsRedirect ? 'Enabled' : 'Disabled'}`
    );
    console.log(
      `🔄 Automatic Renewal: ${summary.features.automaticRenewal ? 'Enabled' : 'Disabled'}`
    );
  ...
  displayFinalResults(summary) {
    ...
    if (
      this.setupResults.ssl?.status === 'completed' &&
    ...
    console.log(
      `  openssl s_client -connect ${this.customDomain}:443 -servername ${this.customDomain}`
    );
    ...
    if (this.setupResults.overall === 'pending_validation') {
      ...
      console.log(
        '   Please add the DNS validation records and run this script again.'
      );
    }
  }
  ...
  async run() {
    try {
      console.log(
        '🚀 Starting complete SSL certificate and custom domain setup...'
      );
      console.log(`Environment: ${this.environment}`);
      console.log(`Custom Domain: ${this.customDomain}`);
      console.log(`CloudFront Distribution: ${this.distributionId}`);
      ...
      console.error(
        '1. Verify AWS credentials have ACM, CloudFront, and Route 53 permissions'
      );
      console.error(
        '2. Ensure CloudFront distribution exists and is accessible'
      );
      console.error('3. Check that custom domain format is correct');
      console.error(
        '4. Verify DNS provider supports the required record types'
      );
