...
function generateTestReport(results) {
    ...
    testCategories: [
      {
        ...
        description:
          'Tests desktop navigation visibility and hamburger menu hiding',
  ...
  report.summary.passedTests = allResults.filter(
    result => result === 'passed' || result === true
  ).length;
  report.summary.failedTests = allResults.filter(
    result => result === 'failed' || result === false
  ).length;

  const reportPath = path.join(
    __dirname,
    '../navigation-behavior-test-results.json'
  );
...
async function main() {
  ...
  console.log(
    'This test suite validates navigation behavior across different devices and screen sizes'
  );
  console.log(
    'Testing requirements 6.1, 6.2, 6.3, and 6.4 from the specification.\n'
  );
