...
class ComprehensiveTestSuite {
  ...
  async runTestSuite(suiteName, testFunction) {
    ...
    try {
      ...
      this.log(`✗ Test suite failed: ${suiteName}: ${error.message} (${duration}ms)`, 'error');
      return suite;
    }
  }
  ...
  async runEndToEndDeploymentTests(suite) {
    ...
    await this.runTest(suite, 'Infrastructure Setup Validation', async () => {
      try {
        ...
        const output = execSync('node scripts/setup-infrastructure.js --dry-run', {
          encoding: 'utf8',
          timeout: 60000
        });
        
        return { status: 'infrastructure_valid', output: output.substring(0, 200) };
      } catch (error) {
        throw new Error(`Infrastructure setup validation failed: ${error.message}`);
      }
    });
    ...
    await this.runTest(suite, 'Deployment Script Validation', async () => {
      try {
        ...
        return { status: 'deployment_script_valid', output: output.substring(0, 200) };
      } catch (error) {
    ...
    await this.runTest(suite, 'Rollback Mechanism Validation', async () => {
      try {
        ...
        throw new Error(`Rollback mechanism validation failed: ${error.message}`);
      }
    });

    if (!this.skipServer) {
      await this.runTest(suite, 'Site Accessibility Post-Deployment', async () => {
        ...
        const hasMainContent = response.body.includes('<main') || response.body.includes('id="main"');
      ...
      await this.runTest(suite, 'Critical User Journeys', async () => {
        try {
          const output = execSync('npx playwright test e2e/critical-user-journeys.spec.ts --reporter=json', {
            encoding: 'utf8',
            timeout: 120000
          });
          ...
          if (results.stats && results.stats.failed > 0) {
            throw new Error(`Critical user journey tests failed: ${results.stats.failed} failures`);
          }
      ...
      await this.runTest(suite, 'Site Accessibility Post-Deployment', async () => {
        throw new Error('SKIP: Server not running, cannot test site accessibility');
      });
  ...
  async runPerformanceValidationTests(suite) {
    if (this.skipServer) {
      await this.runTest(suite, 'Core Web Vitals Validation', async () => {
        throw new Error('SKIP: Server not running, cannot test Core Web Vitals');
      });

      await this.runTest(suite, 'Performance Budget Compliance', async () => {
        throw new Error('SKIP: Server not running, cannot test performance budgets');
      });

      await this.runTest(suite, 'Resource Loading Performance', async () => {
        throw new Error('SKIP: Server not running, cannot test resource loading');
      });
    ...
    await this.runTest(suite, 'Core Web Vitals Validation', async () => {
      try {
        const output = execSync('npx playwright test e2e/performance.spec.ts --grep "Core Web Vitals" --reporter=json', {
          encoding: 'utf8',
          timeout: 120000
        });
        ...
        if (results.stats && results.stats.failed > 0) {
          throw new Error(`Core Web Vitals tests failed: ${results.stats.failed} failures`);
        }
    ...
    await this.runTest(suite, 'Performance Budget Compliance', async () => {
      try {
        const output = execSync('npx playwright test e2e/performance.spec.ts --grep "Performance Budget" --reporter=json', {
          encoding: 'utf8',
          timeout: 120000
        });
        ...
        if (results.stats && results.stats.failed > 0) {
          throw new Error(`Performance budget tests failed: ${results.stats.failed} failures`);
        }
    ...
    await this.runTest(suite, 'Resource Loading Performance', async () => {
      ...
      const hasCaching = response.headers['cache-control'] || response.headers['etag'];
      
      // Validate against budgets
      if (loadTime > this.performanceBudgets.pageLoadTime) {
        throw new Error(`Page load time ${loadTime}ms exceeds budget ${this.performanceBudgets.pageLoadTime}ms`);
      }
    ...
    await this.runTest(suite, 'Bundle Size Analysis', async () => {
      ...
      // Validate against budgets
      if (totalJSSize > this.performanceBudgets.jsBundle * 2) { // Allow 2x for total
        throw new Error(`Total JS bundle size ${totalJSSize} exceeds budget ${this.performanceBudgets.jsBundle * 2}`);
      }
      
      if (totalCSSSize > this.performanceBudgets.cssBundle) {
        throw new Error(`Total CSS bundle size ${totalCSSSize} exceeds budget ${this.performanceBudgets.cssBundle}`);
      }
  ...
  async runSecurityConfigurationTests(suite) {
    await this.runTest(suite, 'Security Headers Validation', async () => {
      try {
        ...
        return { status: 'security_headers_valid', output: output.substring(0, 200) };
      } catch (error) {
    ...
    await this.runTest(suite, 'Access Control Validation', async () => {
      try {
        ...
        return { status: 'access_control_valid', output: output.substring(0, 200) };
      } catch (error) {
    ...
    await this.runTest(suite, 'Penetration Testing Suite', async () => {
      try {
        ...
        return { status: 'penetration_tests_passed', output: output.substring(0, 200) };
      } catch (error) {
    ...
    if (!this.skipServer) {
      await this.runTest(suite, 'Runtime Security Validation', async () => {
        ...
        if (presentHeaders.length < 3) {
          throw new Error(`Insufficient security headers: only ${presentHeaders.length} found`);
        }
      ...
      await this.runTest(suite, 'Runtime Security Validation', async () => {
        throw new Error('SKIP: Server not running, cannot test runtime security');
      });
    }
  }
  ...
  async runInfrastructureIntegrityTests(suite) {
    await this.runTest(suite, 'AWS Configuration Validation', async () => {
      try {
        ...
        throw new Error(`AWS configuration validation failed: ${error.message}`);
      }
    });

    await this.runTest(suite, 'CloudFront Distribution Validation', async () => {
      try {
        const output = execSync('node scripts/cloudfront-security-validator.js', {
          encoding: 'utf8',
          timeout: 60000
        });
        
        return { status: 'cloudfront_valid', output: output.substring(0, 200) };
      } catch (error) {
        throw new Error(`CloudFront validation failed: ${error.message}`);
      }
    });

    await this.runTest(suite, 'Monitoring and Alerting Validation', async () => {
      try {
        ...
        return { status: 'monitoring_valid', output: output.substring(0, 200) };
      } catch (error) {
    ...
    await this.runTest(suite, 'Backup and Recovery Validation', async () => {
      try {
        ...
        return { status: 'backup_recovery_valid', output: output.substring(0, 200) };
      } catch (error) {
        throw new Error(`Backup and recovery validation failed: ${error.message}`);
      }
    });
  }
  ...
  async runAccessibilityComplianceTests(suite) {
    if (this.skipServer) {
      ...
      await this.runTest(suite, 'Keyboard Navigation', async () => {
        throw new Error('SKIP: Server not running, cannot test keyboard navigation');
      });

      await this.runTest(suite, 'Screen Reader Compatibility', async () => {
        throw new Error('SKIP: Server not running, cannot test screen reader compatibility');
      });
    ...
    await this.runTest(suite, 'WCAG 2.1 AA Compliance', async () => {
      try {
        const output = execSync('npx playwright test e2e/accessibility.spec.ts --grep "WCAG 2.1 AA" --reporter=json', {
          encoding: 'utf8',
          timeout: 120000
        });
        ...
        if (results.stats && results.stats.failed > 0) {
          throw new Error(`WCAG compliance tests failed: ${results.stats.failed} failures`);
        }
    ...
    await this.runTest(suite, 'Keyboard Navigation', async () => {
      try {
        const output = execSync('npx playwright test e2e/accessibility.spec.ts --grep "Keyboard Navigation" --reporter=json', {
          encoding: 'utf8',
          timeout: 120000
        });
    ...
    await this.runTest(suite, 'Screen Reader Compatibility', async () => {
      try {
        const output = execSync('npx playwright test e2e/accessibility.spec.ts --grep "Screen Reader" --reporter=json', {
          encoding: 'utf8',
          timeout: 120000
        });
        ...
        if (results.stats && results.stats.failed > 0) {
          throw new Error(`Screen reader tests failed: ${results.stats.failed} failures`);
        }
  ...
  async generateReport() {
    ...
    const jsonReport = path.join(this.outputDir, 'comprehensive-test-report.json');
    fs.writeFileSync(jsonReport, JSON.stringify(this.results, null, 2));
    
    // Generate HTML report
    const htmlReport = this.generateHtmlReport();
    const htmlReportPath = path.join(this.outputDir, 'comprehensive-test-report.html');
  ...
  generateHtmlReport() {
    const { totalSuites, totalTests, passed, failed, skipped, warnings } = this.results.summary;
...
<body>
    <div class="container">
        ...
        <div class="content">
            ...
            <div class="test-suites">
                ...
                ${this.results.testSuites.map((suite, suiteIndex) => `
                    <div class="suite">
  ...
  async run() {
    ...
    try {
      ...
      this.log(`Passed: ${passed} | Failed: ${failed} | Skipped: ${skipped} | Warnings: ${warnings}`);
      this.log(`Success rate: ${totalTests > 0 ? ((passed / totalTests) * 100).toFixed(1) : 0}%`);
      this.log(`${'='.repeat(80)}`);
      
      if (failed > 0) {
        this.log(`\n❌ COMPREHENSIVE TEST SUITE FAILED with ${failed} test failures`);
        this.log(`Review the detailed report for failure analysis: ${reportInfo.html}`);
        process.exit(1);
      } else {
      ...
      this.log(`Fatal error during comprehensive testing: ${error.message}`, 'error');
      process.exit(1);
    }
  }
}
...
if (require.main === module) {
  ...
  console.log('🚀 Starting Comprehensive Test Suite for S3 + CloudFront Deployment');
  if (options.verbose) {
  ...
  (async () => {
    try {
      ...
      console.error('❌ Comprehensive test suite execution failed:', error.message);
      process.exit(1);
    }
  })();
}
