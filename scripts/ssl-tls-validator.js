...
class SSLTLSValidator {
    ...
    async validateTLSConfiguration() {
        ...
        for (const version of tlsVersions) {
            try {
                ...
                this.results.tls.tests[`tls_${version.replace('.', '_').toLowerCase()}`] = {
                    status: supported ? 
                        (isWeak ? 'failed' : 'passed') : 
                        (isSecure ? 'warning' : 'passed'),
                    supported,
                    message: supported ? 
                        (isWeak ? `Weak ${version} is supported - security risk` : `${version} is supported`) :
                        (isSecure ? `${version} is not supported (recommended to enable)` : `${version} is disabled (good)`)
                };
            } catch (error) {
                this.results.tls.tests[`tls_${version.replace('.', '_').toLowerCase()}`] = {
                    status: 'error',
    ...
    validateSubjectName(cert, hostname) {
        ...
        if (cert.subjectaltname) {
            const altNames = cert.subjectaltname.split(', ').map(name => 
                name.replace('DNS:', '').replace('IP Address:', '')
            );
    ...
    validateCertificateTransparency(cert) {
        ...
        const hasCTExtension = cert.ext_key_usage && 
    ...
    async validateCipherSuites() {
        try {
            ...
            const isStrong = strongCiphers.some(strong => cipherInfo.cipher.includes(strong));
    ...
    async testHTTPSRedirect() {
        try {
            ...
            const redirectsToHTTPS = response.headers.location && 
            ...
            return {
                ...
                message: isRedirect && redirectsToHTTPS ?
    ...
    async testMixedContent() {
        try {
            ...
            return {
                ...
                message: httpResources.length === 0 ?
    ...
    async testHTTPSEnforcement() {
        try {
            ...
            const httpsResponse = await this.makeHTTPSRequest(`https://${this.hostname}`);
    ...
    async testOCSPStapling() {
        ...
        return {
            status: 'warning',
            message: 'OCSP stapling test not implemented (requires advanced TLS inspection)'
        };
    }
    ...
    async testPerfectForwardSecrecy() {
        try {
            const cipherInfo = await this.getCipherInfo();
            const supportsPFS = cipherInfo.cipher.includes('ECDHE') || 
    ...
    async generateReport() {
        ...
        const reportPath = path.join(process.cwd(), 'ssl-tls-validation-report.json');
        await fs.writeFile(reportPath, JSON.stringify(report, null, 2));

        // Generate summary
        const summary = this.generateSummaryReport(report);
        const summaryPath = path.join(process.cwd(), 'ssl-tls-validation-summary.md');
