...
class CacheBehaviorVerifier {
  ...
  async testCacheBehavior(url) {
    ...
    try {
      ...
      // Analyze results
      const totalResponseTime = results.requests.reduce(
        (sum, req) => sum + req.responseTime,
        0
      );
      results.averageResponseTime = Math.round(
        totalResponseTime / results.requests.length
      );

      const cacheHits = results.requests.filter(
        req => req.cacheHit || (req.xCache && req.xCache.includes('Hit'))
      ).length;
      results.cacheHitRate = (
        (cacheHits / results.requests.length) *
        100
      ).toFixed(1);
      ...
      const averageSubsequentTime =
        subsequentRequests.reduce((sum, req) => sum + req.responseTime, 0) /
      ...
      console.log(
        `   📊 Cache effective: ${results.cacheEffective ? 'Yes' : 'No'}`
      );
      console.log('');
  ...
  async verifyCachePolicies() {
    ...
    const totalCacheHitRate = testResults.tests.reduce(
      (sum, test) => sum + parseFloat(test.cacheHitRate || 0),
      0
    );
    testResults.summary.averageCacheHitRate = (
      totalCacheHitRate / testResults.tests.length
    ).toFixed(1);
    testResults.summary.overallEffective =
      testResults.summary.effectiveCache >=
      testResults.summary.totalTests * 0.6; // 60% threshold
  ...
  generateVerificationReport(testResults) {
    ...
    console.log(
      `   Effective Caching: ${testResults.summary.effectiveCache}/${testResults.summary.totalTests}`
    );
    console.log(
      `   Average Cache Hit Rate: ${testResults.summary.averageCacheHitRate}%`
    );
    console.log(
      `   Overall Effective: ${testResults.summary.overallEffective ? 'Yes' : 'No'}`
    );
    console.log('');
    ...
    testResults.tests.forEach((test, index) => {
      const fileType = test.url.includes('/images/')
        ? 'Image'
        : test.url.includes('.html')
          ? 'HTML'
          : test.url.includes('/_next/')
            ? 'Static Asset'
            : 'Other';
      ...
      console.log(
        `      Cache Effective: ${test.cacheEffective ? 'Yes' : 'No'}`
      );
      ...
      if (test.requests && test.requests.length > 0) {
        ...
        console.log(
          `      Improvement: ${(((firstRequest.responseTime - lastRequest.responseTime) / firstRequest.responseTime) * 100).toFixed(1)}%`
        );
      }
      console.log('');
    });
    ...
    const imageTest = testResults.tests.find(test =>
      test.url.includes('/images/')
    );
    const htmlTest = testResults.tests.find(test => test.url.includes('.html'));

    if (imageTest) {
      const imagesCached =
        imageTest.cacheEffective || parseFloat(imageTest.cacheHitRate) > 50;
      console.log(
        `   ${imagesCached ? '✅' : '⚠️'} Requirement 4.4 (Images long-term caching): ${imagesCached ? 'Effective' : 'Needs attention'}`
      );
    }

    if (htmlTest) {
      const htmlCached =
        htmlTest.cacheEffective || parseFloat(htmlTest.cacheHitRate) > 0;
      console.log(
        `   ${htmlCached ? '✅' : '⚠️'} Requirement 4.5 (HTML short-term caching): ${htmlCached ? 'Effective' : 'Needs attention'}`
      );
    }
