...
class PerfectForwardSecrecyTester {
  ...
  async testECDHESupport(domain) {
    ...
    const ecdheCiphers = this.pfsCiphers.filter(cipher =>
    ...
    console.log(
      `✅ ECDHE Support: ${results.supportedCiphers.length} ciphers supported`
    );
    console.log(
      `❌ ECDHE Unsupported: ${results.unsupportedCiphers.length} ciphers`
    );
  ...
  async testDHESupport(domain) {
    ...
    const dheCiphers = this.pfsCiphers.filter(cipher =>
      cipher.startsWith('DHE')
    );
    const results = {
      supportedCiphers: [],
      unsupportedCiphers: [],
    ...
    console.log(
      `✅ DHE Support: ${results.supportedCiphers.length} ciphers supported`
    );
    console.log(
      `❌ DHE Unsupported: ${results.unsupportedCiphers.length} ciphers`
    );
  ...
  async testEphemeralKeyGeneration(domain) {
    ...
    try {
      ...
      results.testPassed =
        results.ephemeralKeySupport && results.uniqueKeys.size > 0;
      this.results.pfsCompliance.ephemeralKeyGeneration = results.testPassed;

      console.log(
        `✅ Ephemeral Keys Generated: ${results.connections.length}/5 connections`
      );
  ...
  async testPFSCompliance(domain) {
    ...
    try {
      ...
      if (this.isPFSCipher(defaultConnection.cipher)) {
        ...
        console.log(
          `✅ Default connection uses PFS: ${defaultConnection.cipher}`
        );
      } else {
        ...
        console.log(
          `❌ Default connection does NOT use PFS: ${defaultConnection.cipher}`
        );
      }

      // Test with specific PFS ciphers
      for (const cipher of this.pfsCiphers.slice(0, 3)) {
        // Test top 3 PFS ciphers
        try {
      ...
      results.compliancePercentage =
        (results.pfsEnabledConnections / results.totalConnections) * 100;
      results.testPassed = results.compliancePercentage >= 80; // 80% compliance threshold

      console.log(
        `📊 PFS Compliance: ${results.compliancePercentage.toFixed(1)}%`
      );
      console.log(
        `✅ PFS Connections: ${results.pfsEnabledConnections}/${results.totalConnections}`
      );
    } catch (error) {
  ...
  generateReport() {
    ...
    if (!this.results.pfsCompliance.ecdheCipherSupport) {
      report.recommendations.push({
        ...
        recommendation:
          'Enable ECDHE cipher suites in CloudFront distribution for better PFS support',
      });
    }
    ...
    if (!this.results.pfsCompliance.dheCipherSupport) {
      report.recommendations.push({
        ...
        recommendation:
          'Consider enabling DHE cipher suites as fallback for ECDHE',
      });
    }
    ...
    if (!this.results.pfsCompliance.ephemeralKeyGeneration) {
      report.recommendations.push({
        ...
        recommendation:
          'Verify CloudFront configuration supports ephemeral key generation',
      });
    }
  ...
  async runAllTests(domain = 'github.com') {
    ...
    try {
      // Test ECDHE support
      this.results.testResults.ecdheSupport =
        await this.testECDHESupport(domain);
      ...
      this.results.testResults.ephemeralKeyGeneration =
        await this.testEphemeralKeyGeneration(domain);
      this.results.summary.totalTests++;
      ...
      this.results.testResults.pfsCompliance =
        await this.testPFSCompliance(domain);
      this.results.summary.totalTests++;
      ...
      console.log(
        `❌ Tests Failed: ${this.results.summary.failed}/${this.results.summary.totalTests}`
      );
      console.log(`🛡️ Security Level: ${report.securityLevel}`);
      console.log(
        `🔐 Overall PFS Compliance: ${report.pfsCompliance.overallCompliance ? 'COMPLIANT' : 'NON-COMPLIANT'}`
      );
