...
class HttpsRedirectValidator {
    ...
    async testHttpToHttpsRedirect(domain) {
        ...
        for (const url of testUrls) {
            try {
                const response = await this.makeHttpRequest(url, { followRedirects: false });
                
                const testName = `HTTP to HTTPS Redirect - ${url}`;
                const isRedirect = response.statusCode >= 300 && response.statusCode < 400;
                const locationHeader = response.headers.location;
                
                if (isRedirect && locationHeader && locationHeader.startsWith('https://')) {
    ...
    async testRedirectChain(domain) {
        ...
        for (const url of testUrls) {
            try {
                ...
                if (finalUrl && finalUrl.startsWith('https://') && redirectChain.length <= this.options.maxRedirects) {
                    this.addTestResult(testName, 'PASSED', {
                        redirectChain,
                        finalUrl,
            ...
            for (const [headerName, expectedValues] of Object.entries(securityHeaders)) {
                ...
                if (headerValue) {
                    if (expectedValues === null) {
                        ...
                        // Check exact match
                        const isValid = headerValue.toLowerCase() === expectedValues.toLowerCase();
    ...
    validateHstsConfiguration(hstsConfig) {
        ...
        this.addTestResult('HSTS Include Subdomains', hstsConfig.includeSubDomains ? 'PASSED' : 'INFO', {
            includeSubDomains: hstsConfig.includeSubDomains,
            recommendation: hstsConfig.includeSubDomains ? null : 'Consider adding includeSubDomains directive'
        });
    }
    ...
    generateSummary() {
        ...
        this.results.summary.overallStatus = summary.failed === 0 ? 'PASSED' : 'FAILED';
    }
    ...
    generateRecommendations() {
        ...
        const warningTests = this.results.testResults.filter(test => test.status === 'WARNING');

        if (failedTests.some(test => test.test.includes('HTTP to HTTPS Redirect'))) {
            recommendations.push({
                priority: 'HIGH',
                category: 'Security',
                issue: 'HTTP to HTTPS redirect not properly configured',
                recommendation: 'Configure CloudFront to redirect all HTTP requests to HTTPS',
                implementation: 'Set up CloudFront behavior to redirect HTTP to HTTPS with 301 status'
            });
        }
        ...
        if (failedTests.some(test => test.test.includes('HSTS Header'))) {
            recommendations.push({
                ...
                recommendation: 'Add Strict-Transport-Security header to all HTTPS responses',
                implementation: 'Configure CloudFront to add HSTS header with appropriate max-age'
            });
        }
        ...
        if (warningTests.some(test => test.test.includes('Security Header'))) {
            recommendations.push({
                ...
                implementation: 'Configure CloudFront to add security headers like CSP, X-Frame-Options, etc.'
            });
        }
