...
class CloudFrontCachingConfig {
  ...
  async createStaticAssetsCachePolicy() {
    ...
    try {
      ...
      if (error.name === 'CachePolicyAlreadyExists') {
        console.log(
          '⚠️  Static assets cache policy already exists, continuing...'
        );
        return null;
      }
      throw error;
  ...
  async createServiceWorkerCachePolicy() {
    ...
    try {
      ...
      if (error.name === 'CachePolicyAlreadyExists') {
        console.log(
          '⚠️  Service worker cache policy already exists, continuing...'
        );
        return null;
      }
      throw error;
      ...
      CachePolicyConfig: {
        ...
        ParametersInCacheKeyAndForwardedToOrigin: {
          ...
          HeadersConfig: {
            HeaderBehavior: 'whitelist',
            Headers: {
              Items: ['CloudFront-Is-Mobile-Viewer', 'Accept'],
            },
          },
  ...
  async updateDistributionCaching(policies) {
    ...
    console.log(
      'Updating CloudFront distribution with caching configuration...'
    );
    ...
    try {
      ...
      console.log(
        '✅ CloudFront distribution updated with caching configuration'
      );
  ...
  async run() {
    try {
      ...
      console.log('\n🔑 Cache Policy IDs:');
      if (staticAssetsPolicy)
        console.log(`Static Assets: ${staticAssetsPolicy.Id}`);
      if (htmlPolicy) console.log(`HTML Files: ${htmlPolicy.Id}`);
      if (serviceWorkerPolicy)
        console.log(`Service Worker: ${serviceWorkerPolicy.Id}`);
      if (manifestPolicy) console.log(`Manifest: ${manifestPolicy.Id}`);

      return policies;
    } catch (error) {
      console.error(
        '\n❌ CloudFront caching configuration failed:',
        error.message
      );
      process.exit(1);
    }
