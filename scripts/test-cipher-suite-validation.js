...
class CipherSuiteValidationTester {
    ...
    async testDomain(domain) {
        ...
        try {
            ...
            if (results.passed) {
                console.log(`✅ ${domain} - Cipher validation PASSED (Score: ${results.score}/100)`);
                this.testResults.summary.passed++;
            } else {
                console.log(`❌ ${domain} - Cipher validation FAILED (Score: ${results.score}/100)`);
    ...
    generateTestSummary() {
        ...
        // Save test results
        const reportPath = path.join(__dirname, '..', 'config', 'cipher-suite-test-results.json');
    ...
    generateTestRecommendations() {
        ...
        allResults.forEach(result => {
            ...
            result.weakCiphers.forEach(weak => {
                commonWeakCiphers[weak.weakness] = (commonWeakCiphers[weak.weakness] || 0) + 1;
            });
    ...
    async testSpecificScenarios() {
        ...
        try {
            ...
            scenarios.forEach(scenario => {
                ...
                if (!passed) {
                    console.log(`   ⚠️  This scenario failed - review cipher configuration`);
                }
                console.log();
            });
