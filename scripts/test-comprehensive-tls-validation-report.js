...
class ComprehensiveTLSReportTester {
    ...
    async testBasicReportGeneration() {
        ...
        try {
            ...
            const scriptPath = path.join(__dirname, 'comprehensive-tls-validation-report.js');
            ...
            const successCheck = {
                ...
                expected: true,
                actual: output.includes('Comprehensive TLS validation report generation completed successfully'),
                status: 'unknown'
            };
            successCheck.status = successCheck.actual === successCheck.expected ? 'passed' : 'failed';
            testResult.checks.push(successCheck);
            
            console.log(`  Command Execution: ${successCheck.status === 'passed' ? '✅' : '❌'}`);
    ...
    async testReportFileCreation() {
        ...
        try {
            ...
            jsonCheck.status = jsonCheck.actual === jsonCheck.expected ? 'passed' : 'failed';
            testResult.checks.push(jsonCheck);
            console.log(`  JSON Report: ${jsonCheck.status === 'passed' ? '✅' : '❌'}`);
            ...
            mdCheck.status = mdCheck.actual === mdCheck.expected ? 'passed' : 'failed';
            testResult.checks.push(mdCheck);
            console.log(`  Markdown Report: ${mdCheck.status === 'passed' ? '✅' : '❌'}`);
            ...
            htmlCheck.status = htmlCheck.actual === htmlCheck.expected ? 'passed' : 'failed';
            testResult.checks.push(htmlCheck);
            console.log(`  HTML Report: ${htmlCheck.status === 'passed' ? '✅' : '❌'}`);
            
            // Check summary report
            const summaryExists = fs.existsSync(path.join(this.outputDir, 'tls-validation-summary.md'));
            const summaryCheck = {
            ...
            summaryCheck.status = summaryCheck.actual === summaryCheck.expected ? 'passed' : 'failed';
            testResult.checks.push(summaryCheck);
            console.log(`  Summary Report: ${summaryCheck.status === 'passed' ? '✅' : '❌'}`);
    ...
    async testReportContentValidation() {
        ...
        try {
            ...
            if (jsonFiles.length > 0) {
                ...
                structureCheck.status = structureCheck.actual === structureCheck.expected ? 'passed' : 'failed';
                testResult.checks.push(structureCheck);
                console.log(`  Report Structure: ${structureCheck.status === 'passed' ? '✅' : '❌'}`);
                
                // Check metadata
                const metadataCheck = {
                    name: 'Metadata Present',
                    expected: true,
                    actual: reportData.metadata && reportData.metadata.domain === this.testDomain,
                    status: 'unknown'
                };
                metadataCheck.status = metadataCheck.actual === metadataCheck.expected ? 'passed' : 'failed';
                testResult.checks.push(metadataCheck);
                console.log(`  Metadata: ${metadataCheck.status === 'passed' ? '✅' : '❌'}`);
                
                // Check executive summary
                const summaryCheck = {
                    name: 'Executive Summary',
                    expected: true,
                    actual: reportData.executiveSummary && reportData.executiveSummary.overallSecurityRating,
                    status: 'unknown'
                };
                summaryCheck.status = summaryCheck.actual === summaryCheck.expected ? 'passed' : 'failed';
                testResult.checks.push(summaryCheck);
                console.log(`  Executive Summary: ${summaryCheck.status === 'passed' ? '✅' : '❌'}`);
                
                // Check recommendations
                const recommendationsCheck = {
                    name: 'Recommendations Present',
                    expected: true,
                    actual: reportData.recommendations && typeof reportData.recommendations === 'object',
                    status: 'unknown'
                };
                recommendationsCheck.status = recommendationsCheck.actual === recommendationsCheck.expected ? 'passed' : 'failed';
                testResult.checks.push(recommendationsCheck);
                console.log(`  Recommendations: ${recommendationsCheck.status === 'passed' ? '✅' : '❌'}`);
    ...
    async testCommandLineOptions() {
        ...
        try {
            ...
            const helpOutput = execSync(helpCommand, { encoding: 'utf8', timeout: 10000 });
            
            const helpCheck = {
                name: 'Help Option',
                expected: true,
                actual: helpOutput.includes('Usage:') && helpOutput.includes('Options:'),
                status: 'unknown'
            };
            helpCheck.status = helpCheck.actual === helpCheck.expected ? 'passed' : 'failed';
            testResult.checks.push(helpCheck);
            console.log(`  Help Option: ${helpCheck.status === 'passed' ? '✅' : '❌'}`);
            
            // Test no-html option
            const noHtmlCommand = `node "${scriptPath}" ${this.testDomain} --no-html --no-tests --output-dir "${this.outputDir}"`;
            execSync(noHtmlCommand, { encoding: 'utf8', timeout: 60000, stdio: 'pipe' });
            
            // Check that HTML file was not created in this run
            const files = fs.readdirSync(this.outputDir);
            const recentHtmlFiles = files.filter(file => 
                file.startsWith('comprehensive-tls-validation-report-') && 
                file.includes(this.testDomain) &&
                file.endsWith('.html')
            );
            ...
            console.log(`  No-HTML Option: ${noHtmlCheck.status === 'passed' ? '✅' : '❌'}`);
    ...
    async testErrorHandling() {
        ...
        try {
            ...
            try {
                ...
                errorHandlingCheck.status = errorHandlingCheck.actual === errorHandlingCheck.expected ? 'passed' : 'warning';
                testResult.checks.push(errorHandlingCheck);
                console.log(`  Invalid Domain Handling: ${errorHandlingCheck.status === 'passed' ? '✅' : '⚠️'}`);
    ...
    validateReportStructure(reportData) {
        ...
        return requiredSections.every(section => reportData.hasOwnProperty(section));
    }
     ...
     */
    saveTestResults() {
        const resultsFile = path.join(this.outputDir, 'comprehensive-tls-validation-report-test-results.json');
