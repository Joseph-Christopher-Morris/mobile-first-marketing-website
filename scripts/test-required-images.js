  ...
  return new Promise((resolve) => {
    ...
    const req = https.get(url, (res) => {
      ...
      if (res.statusCode === 200 && contentType && contentType.startsWith('image/')) {
        ...
        resolve({ success: true, path: imagePath, status: res.statusCode, contentType });
      } else {
        console.log(`❌ ${imagePath}`);
        console.log(`   Status: ${res.statusCode}`);
        console.log(`   Content-Type: ${contentType}`);
        console.log(`   Expected: image/* content type\n`);
        resolve({ success: false, path: imagePath, status: res.statusCode, contentType });
      }
...
async function testAllImages() {
  ...
  if (failed > 0) {
    ...
    results.filter(r => !r.success).forEach(r => {
      console.log(`   - ${r.path}: ${r.error || `${r.status} ${r.contentType}`}`);
    });
  }
  ...
  if (successful === requiredImages.length) {
    console.log('🎉 All required images are accessible with correct MIME types!');
    process.exit(0);
  } else {
