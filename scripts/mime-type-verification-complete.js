...
// Configuration
const config = {
    s3BucketName: process.env.S3_BUCKET_NAME || 'mobile-marketing-site-prod-1759705011281-tyzuo9',
    cloudfrontDomain: process.env.CLOUDFRONT_DOMAIN || 'd15sc9fc739ev2.cloudfront.net',
...
async function main() {
    ...
    try {
        ...
        const identity = executeAwsCommand(`aws sts get-caller-identity --region ${config.region}`);
            ...
            for (const object of objects.Contents) {
                ...
                try {
                    ...
                    if (result.correct) {
                        ...
                        console.log(`❌ ${key}: Expected ${expectedMimeType}, got ${actualMimeType}`);
                    }
        ...
        for (const file of imageFiles) {
            try {
                ...
                if (result.correct) {
                    console.log(`✅ ${file.key}: ${result.actualMimeType} (${result.cacheStatus})`);
                } else {
                    console.log(`❌ ${file.key}: Expected ${result.expectedMimeType}, got ${result.actualMimeType}`);
                }

            } catch (error) {
                console.error(`❌ Error testing CloudFront response for ${file.key}:`, error.message);
                results.summary.errors++;
            }
        }
        ...
        try {
            ...
            console.log(`   Content-Type: ${s3Metadata.ContentType} ${s3Correct ? '✅' : '❌'}`);
            ...
            console.log(`   Content-Type: ${cfHeaders['content-type']} ${cfCorrect ? '✅' : '❌'}`);
        ...
        console.log(`   Incorrect MIME Types: ${results.summary.incorrectMimeTypes}`);
        console.log(`   Errors: ${results.summary.errors}`);
        
        if (results.summary.totalChecked > 0) {
            const successRate = ((results.summary.correctMimeTypes / results.summary.totalChecked) * 100).toFixed(1);
        ...
        const reportData = {
            ...
            taskCompletion: {
                ...
                cloudfrontMimeTypesCorrect: results.cloudfrontResponses.every(r => r.correct),
                blogImageWorking: results.blogImageTest && results.blogImageTest.s3.correct && results.blogImageTest.cloudfront.correct,
                overallSuccess: results.summary.incorrectMimeTypes === 0 && results.summary.errors === 0
            }
        };
        ...
        console.log(`✅ S3 objects have correct Content-Type headers for WebP files: ${reportData.taskCompletion.s3MimeTypesCorrect ? 'PASSED' : 'FAILED'}`);
        console.log(`✅ CloudFront serves images with proper MIME types: ${reportData.taskCompletion.cloudfrontMimeTypesCorrect ? 'PASSED' : 'FAILED'}`);
        console.log(`✅ Browsers receive correct content-type headers: ${reportData.taskCompletion.blogImageWorking ? 'PASSED' : 'FAILED'}`);
        console.log(`✅ Blog image MIME type verification: ${reportData.taskCompletion.blogImageWorking ? 'PASSED' : 'FAILED'}`);
