...
class CipherSuiteValidator {
    ...
    try {
      ...
      if (actualCipher) {
        ...
        if (this.isStrongCipher(actualCipher.name)) {
          ...
          console.log(
            `⚠️  Negotiated cipher may be weak: ${actualCipher.name}`
          );
        }
      } else {
  ...
  async testStrongCipherSupport(domain) {
    ...
    for (const cipherConfig of cipherConfigs) {
      try {
        ...
        if (result) {
          console.log(
            `✅ Cipher config supported: ${cipherConfig} -> ${result.name}`
          );
  ...
  async validateCipherOrdering(domain) {
    ...
    try {
      ...
      for (const cipherList of cipherTests) {
        try {
          const selectedCipher = await this.testCipherPreference(
            domain,
            cipherList
          );
          ...
          console.log(
            `🔍 Cipher list "${cipherList}" selected: ${selectedCipher}`
          );
        } catch (error) {
      ...
      const uniqueSelections = [
        ...new Set(orderingResults.map(r => r.selectedCipher)),
      ];
      if (uniqueSelections.length === 1) {
        console.log('✅ Server has consistent cipher preference');
      } else {
        console.log(
          '⚠️  Server cipher selection varies - may honor client preference'
        );
      }
    } catch (error) {
  ...
  generateRecommendations() {
    ...
    const hasModernCiphers = this.results.strongCiphers.some(
      cipher => cipher.includes('GCM') || cipher.includes('CHACHA20')
    );
    ...
    if (aes256 === 0 && aes128 > 0) {
      this.results.recommendations.push(
        'Consider adding AES-256 ciphers for enhanced security'
      );
    }

    if (this.results.recommendations.length === 0) {
      this.results.recommendations.push(
        'Cipher suite configuration appears to be well-configured'
      );
    }
  }
  ...
  calculateScore() {
    ...
    const strongCipherRatio =
      this.results.strongCiphers.length / this.strongCipherSuites.length;
    ...
    const { aes256, chacha20, perfectForwardSecrecy } =
      this.results.encryptionStrength;
    if (aes256 > 0) score += 8;
  ...
  generateReport() {
    ...
    const summaryPath = path.join(
      ...
      '..',
      'config',
      'cipher-suite-validation-summary.md'
    );
...
async function main() {
  ...
  try {
    ...
    if (results.passed) {
      ...
      console.log(
        'Please review the recommendations and improve the configuration.'
      );
      process.exit(1);
    }
  } catch (error) {
