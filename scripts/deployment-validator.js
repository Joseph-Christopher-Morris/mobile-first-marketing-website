...
class DeploymentValidator {
  constructor(options = {}) {
    ...
    this.environment =
      options.environment || process.env.DEPLOYMENT_ENVIRONMENT || 'production';
    this.deploymentId =
      options.deploymentId || process.env.GITHUB_SHA || `deploy-${Date.now()}`;
  ...
  async performHealthCheck() {
    ...
    try {
      ...
      if (response.statusCode === 200) {
        this.log(
          `Health check passed - Status: ${response.statusCode}`,
          'success'
        );
  ...
  async validateSecurityHeaders() {
    ...
    if (!this.siteUrl) {
      this.log(
        'Site URL not configured, skipping security validation',
        'warning'
      );
    ...
    try {
      ...
      if (passRate >= 0.75) {
        this.log(
          `Security headers validation passed (${passed}/${total})`,
          'success'
        );
        ...
        this.log(
          `Security headers validation failed (${passed}/${total})`,
          'warning'
        );
  ...
  async validatePerformance() {
    ...
    if (!this.siteUrl) {
      this.log(
        'Site URL not configured, skipping performance validation',
        'warning'
      );
    ...
    try {
      ...
      if (responseTime <= thresholds.responseTime) {
        this.log(
          `✓ Response time: ${responseTime}ms (under ${thresholds.responseTime}ms)`,
          'success'
        );
      } else {
        this.log(
          `✗ Response time: ${responseTime}ms (over ${thresholds.responseTime}ms)`,
          'warning'
        );
        passed = false;
      }
      ...
      if (contentSize <= thresholds.contentSize) {
        this.log(
          `✓ Content size: ${this.formatBytes(contentSize)} (under ${this.formatBytes(thresholds.contentSize)})`,
          'success'
        );
      } else {
        this.log(
          `✗ Content size: ${this.formatBytes(contentSize)} (over ${this.formatBytes(thresholds.contentSize)})`,
          'warning'
        );
        passed = false;
      }
      ...
      if (
        compressionHeader &&
        (compressionHeader.includes('gzip') || compressionHeader.includes('br'))
      ) {
  ...
  async validateFunctionality() {
    ...
    try {
      ...
      execSync(
        'node scripts/validate-site-functionality-simple.js --skip-server',
        {
          stdio: 'pipe',
          env: {
            ...process.env,
  ...
  async sendNotifications(notification) {
    ...
    this.log(
      'Notification generated (external services not configured)',
      'info'
    );
  ...
  async triggerRollback() {
    this.log(
      'Critical deployment failure detected, triggering rollback...',
      'error'
    );
