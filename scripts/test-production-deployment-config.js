...
function loadProductionEnv() {
  ...
  if (!fs.existsSync(envPath)) {
    throw new Error('Production environment file not found: config/production.env');
  }
...
async function testProductionDeploymentConfig() {
  ...
  try {
    ...
    for (const imageFile of imageFiles) {
      ...
      switch (ext) {
        case '.webp': expectedMimeType = 'image/webp'; break;
        case '.jpg':
        case '.jpeg': expectedMimeType = 'image/jpeg'; break;
        case '.png': expectedMimeType = 'image/png'; break;
        case '.gif': expectedMimeType = 'image/gif'; break;
        case '.svg': expectedMimeType = 'image/svg+xml'; break;
        case '.ico': expectedMimeType = 'image/x-icon'; break;
        case '.avif': expectedMimeType = 'image/avif'; break;
        default: expectedMimeType = 'application/octet-stream';
      }
      
      const mimeCorrect = contentType === expectedMimeType;
      const cacheCorrect = cacheHeaders['Cache-Control'] === 'public, max-age=31536000';
      
      // Test invalidation logic
      const isImageFile = /\.(webp|jpg|jpeg|png|gif|svg|ico|avif)$/i.test(imageFile);
      ...
      console.log(`${mimeCorrect && cacheCorrect && willBeInvalidated ? '✅' : '❌'} ${path.basename(imageFile)}`);
      console.log(`   MIME: ${contentType} ${mimeCorrect ? '✅' : '❌'}`);
      console.log(`   Cache: ${cacheHeaders['Cache-Control']} ${cacheCorrect ? '✅' : '❌'}`);
    ...
    console.log(`   MIME types correct: ${mimeTestsPassed}/${imageFiles.length}`);
    console.log(`   Cache headers correct: ${cacheTestsPassed}/${imageFiles.length}`);
    console.log(`   Will be invalidated: ${invalidationTestsPassed}/${imageFiles.length}`);
    ...
    const scenarios = [
      {
        ...
        files: ['images/services/photography-hero.webp', 'images/services/analytics-hero.webp', 'images/services/ad-campaigns-hero.webp']
      },
      {
        name: 'Blog preview images (Req 1.2)', 
        files: ['images/hero/google-ads-analytics-dashboard.webp', 'images/hero/whatsapp-image-2025-07-11-flyers-roi.webp', 'images/hero/240619-london-19.webp']
      },
      {
        name: 'Service page hero images (Req 2.1, 2.3, 2.5)',
        files: ['images/services/250928-hampson-auctions-sunday-11.webp', 'images/services/screenshot-2025-09-23-analytics-dashboard.webp', 'images/services/ad-campaigns-hero.webp']
      },
      {
    ...
    for (const scenario of scenarios) {
      ...
      for (const file of scenario.files) {
        if (imageFiles.includes(file)) {
          ...
          const fileSuccess = contentType.startsWith('image/') && 
                             cacheHeaders['Cache-Control'] === 'public, max-age=31536000' &&
                             willBeInvalidated;
          
          console.log(`     ${fileSuccess ? '✅' : '❌'} ${path.basename(file)}`);
    ...
    const allTestsPassed = mimeTestsPassed === imageFiles.length && 
                          cacheTestsPassed === imageFiles.length &&
                          invalidationTestsPassed === imageFiles.length &&
                          scenariosPassed === scenarios.length;
    
    console.log(`   MIME Types: ${mimeTestsPassed === imageFiles.length ? '✅' : '❌'} (${mimeTestsPassed}/${imageFiles.length})`);
    console.log(`   Cache Headers: ${cacheTestsPassed === imageFiles.length ? '✅' : '❌'} (${cacheTestsPassed}/${imageFiles.length})`);
    console.log(`   Cache Invalidation: ${invalidationTestsPassed === imageFiles.length ? '✅' : '❌'} (${invalidationTestsPassed}/${imageFiles.length})`);
    console.log(`   Requirements Scenarios: ${scenariosPassed === scenarios.length ? '✅' : '❌'} (${scenariosPassed}/${scenarios.length})`);
  ...
  testProductionDeploymentConfig()
    .then((passed) => {
      if (passed) {
        ...
        console.log('✅ 11.2 CloudFront cache invalidation for images implemented');
        console.log('\nThe deployment script now:');
        console.log('  • Sets correct Content-Type headers for all image formats');
