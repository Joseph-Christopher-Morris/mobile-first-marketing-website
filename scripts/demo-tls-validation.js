...
async function demoTLSValidation() {
    ...
    for (const test of testDomains) {
        ...
        try {
            ...
            console.log(`   - TLS 1.2 Support: ${results.tlsVersions['TLS 1.2']?.supported ? '✅' : '❌'}`);
            console.log(`   - TLS 1.3 Support: ${results.tlsVersions['TLS 1.3']?.supported ? '✅' : '❌'}`);
            console.log(`   - Perfect Forward Secrecy: ${results.securityAnalysis?.pfs ? '✅' : '❌'}`);
            console.log(`   - Strong Encryption: ${results.securityAnalysis?.strongEncryption ? '✅' : '❌'}`);
            console.log(`   - Security Level: ${results.securityAnalysis?.securityLevel || 'Unknown'}`);
    ...
    console.log('node tls-version-cipher-validator.js your-domain.com --port 443 --output results.json');
}
