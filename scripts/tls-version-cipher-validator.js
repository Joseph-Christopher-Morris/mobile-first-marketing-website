...
class TLSValidator {
        ...
        for (const tlsVersion of tlsVersions) {
            try {
                ...
                this.results.tlsVersions[tlsVersion.name] = {
                    ...
                    status: supported ? (tlsVersion.secure ? '✅ GOOD' : '⚠️  INSECURE') : '❌ NOT SUPPORTED'
                };
                
                console.log(`  ${tlsVersion.name}: ${this.results.tlsVersions[tlsVersion.name].status}`);
    ...
    async analyzeCipherSuites() {
        ...
        try {
            ...
            console.log(`  Perfect Forward Secrecy: ${cipherInfo.analysis.pfs ? '✅ YES' : '❌ NO'}`);
            console.log(`  Strong encryption: ${cipherInfo.analysis.strongEncryption ? '✅ YES' : '❌ NO'}`);
    ...
    analyzeCipherSecurity(cipher) {
        ...
        if (cipher.name) {
            ...
            if (cipherName.includes('AES256') || cipherName.includes('AES_256') || cipherName.includes('AES-256')) {
                analysis.encryption = 'AES-256';
            } else if (cipherName.includes('AES128') || cipherName.includes('AES_128') || cipherName.includes('AES-128')) {
    ...
    generateRecommendations() {
        ...
        const tlsSecure = this.results.tlsVersions['TLS 1.2']?.supported || 
                         this.results.tlsVersions['TLS 1.3']?.supported;
        const tlsInsecure = this.results.tlsVersions['TLS 1.0']?.supported || 
                           this.results.tlsVersions['TLS 1.1']?.supported;
        
        if (!tlsSecure) {
            this.results.recommendations.push('Enable TLS 1.2 and/or TLS 1.3 support');
    ...
    saveResults(filename) {
        const outputFile = filename || `tls-validation-${this.domain}-${Date.now()}.json`;
  ...
  node tls-version-cipher-validator.js mysite.com --output tls-results.json
    ...
    validator.validate()
        .then((results) => {
            ...
            const hasInsecureTLS = results.tlsVersions['TLS 1.0']?.supported || 
                                  results.tlsVersions['TLS 1.1']?.supported;
            const hasSecureTLS = results.tlsVersions['TLS 1.2']?.supported || 
