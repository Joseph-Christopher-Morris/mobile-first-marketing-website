...
class ImagePerformanceMonitor {
  ...
  assessCoreWebVitalsImpact() {
    ...
    const failedImages = this.results.imageMetrics.filter(m => !m.success).length;
    const slowImages = this.results.imageMetrics.filter(m => m.loadTime > 2500).length;
    const avgLoadTime = this.results.imageMetrics.reduce((sum, m) => sum + m.loadTime, 0) / totalImages;
    const totalImageSize = this.results.imageMetrics.reduce((sum, m) => sum + m.sizeKB, 0);
    
    // Estimate LCP impact (Largest Contentful Paint)
    const heroImages = this.results.imageMetrics.filter(m => 
      m.path.includes('/hero/') || m.path.includes('hero') || m.path.includes('A7302858')
    );
    const maxHeroLoadTime = Math.max(...heroImages.map(m => m.loadTime), 0);
    
    // Estimate CLS impact (Cumulative Layout Shift)
    const clsRisk = failedImages > 0 ? 'HIGH' : slowImages > 2 ? 'MEDIUM' : 'LOW';
    
    this.results.coreWebVitals = {
      lcp: {
        estimatedImpact: maxHeroLoadTime,
        threshold: 2500,
        status: maxHeroLoadTime > 2500 ? 'POOR' : maxHeroLoadTime > 1200 ? 'NEEDS_IMPROVEMENT' : 'GOOD',
        recommendation: maxHeroLoadTime > 2500 ? 'Optimize hero images for faster loading' : 'Hero image performance is acceptable'
      },
      cls: {
        riskLevel: clsRisk,
        failedImages,
        recommendation: failedImages > 0 ? 'Fix failed image loading to prevent layout shifts' : 'Image loading stability is good'
      },
      fid: {
        impact: 'MINIMAL',
        recommendation: 'Image loading should not significantly impact First Input Delay'
      },
      performance: {
        avgLoadTime: Math.round(avgLoadTime),
        totalSize: totalImageSize,
        failureRate: Math.round((failedImages / totalImages) * 100),
        recommendation: avgLoadTime > 2000 ? 'Consider image optimization and CDN improvements' : 'Image performance is acceptable'
      }
    };
  }
  ...
  generateSummary() {
    const totalImages = this.results.imageMetrics.length;
    const successfulImages = this.results.imageMetrics.filter(m => m.success).length;
    const failedImages = totalImages - successfulImages;
    const avgLoadTime = this.results.imageMetrics.reduce((sum, m) => sum + m.loadTime, 0) / totalImages;
    const totalSize = this.results.imageMetrics.reduce((sum, m) => sum + m.sizeKB, 0);
    
    const errorCount = this.results.alerts.filter(a => a.severity === 'ERROR').length;
    const warningCount = this.results.alerts.filter(a => a.severity === 'WARNING').length;
  ...
  generateRecommendations() {
    ...
    const noCacheImages = this.results.imageMetrics.filter(m => 
      !m.cacheControl || !m.cacheControl.includes('max-age')
    );
...
|------------|--------|-----------|------|--------------|
${this.results.imageMetrics.map(metric => 
  ...
  async runMonitoring() {
    ...
    for (const imagePath of this.criticalImages) {
      ...
      if (metric.success) {
        ...
        console.log(`  ❌ Failed: ${metric.error || `HTTP ${metric.statusCode}`}`);
      }
    }
