...
class PFSValidator {
  ...
  async validateMultipleDomains(domains = null) {
    ...
    for (const domain of testDomains) {
      ...
      try {
        ...
        results[domain] = {
          status: domainResults.pfsCompliance.overallCompliance
            ? 'PASS'
            : 'FAIL',
        ...
        console.log(
          `✅ ${domain}: ${results[domain].status} (${results[domain].securityLevel})`
        );
      } catch (error) {
        results[domain] = {
          status: 'ERROR',
  ...
  async validateCloudFrontPFS(distributionDomain) {
    ...
    const supportedCiphers =
      results.testResults.ecdheSupport.supportedCiphers.map(c => c.cipher);

    cloudFrontValidation.expectedCiphersSupported = expectedCiphers.some(
      cipher => supportedCiphers.includes(cipher)
    );
  ...
  generateValidationReport(domainResults, cloudFrontResults = null) {
    ...
    if (report.summary.failedDomains > 0) {
      report.recommendations.push({
        ...
        recommendation:
          'Review and update TLS configuration to support Perfect Forward Secrecy',
      });
    }
