...
class SecurityValidator {
    ...
    async validateSecurityHeaders(url) {
        ...
        try {
            ...
            for (const [headerName, config] of Object.entries(requiredHeaders)) {
                ...
                this.log(`${test.status.toUpperCase()}: ${test.message}`, test.status === 'failed' ? 'error' : 'info');
            }
            ...
            for (const header of dangerousHeaders) {
                if (headers[header]) {
                    ...
                    this.results.overall.warnings++;
                    this.log(`WARNING: Information disclosure header '${header}' found`, 'error');
                }
            }
    ...
    async validateSSLConfiguration(hostname) {
        ...
        try {
            ...
            this.results.ssl.tests.certificate_subject = {
                status: certInfo.subject.CN === hostname || certInfo.subjectaltname?.includes(hostname) ? 'passed' : 'failed',
    ...
    async testDirectoryTraversal(baseUrl) {
        ...
        for (const payload of payloads) {
            try {
                ...
                if (response.statusCode === 200 && 
                    (response.body.includes('root:') || response.body.includes('[drivers]'))) {
                    vulnerable = true;
        ...
        this.results.penetration.tests.directory_traversal = {
            ...
            message: vulnerable ? 'Directory traversal vulnerability detected' : 'No directory traversal vulnerabilities found',
            payloads: responses
        };
    }
    ...
    async testSQLInjection(baseUrl) {
        ...
        for (const payload of payloads) {
            try {
                const response = await this.makeHttpsRequest(`${baseUrl}/?id=${encodeURIComponent(payload)}`, {
                    method: 'GET',
                    timeout: 5000
                });
        ...
        this.results.penetration.tests.sql_injection = {
            ...
            message: suspicious ? 'Potential SQL injection vectors found' : 'No SQL injection vulnerabilities detected (static site)',
            payloads: responses
        };
    }
    ...
    async testXSSVulnerabilities(baseUrl) {
        ...
        for (const payload of payloads) {
            try {
                const response = await this.makeHttpsRequest(`${baseUrl}/?q=${encodeURIComponent(payload)}`, {
                    method: 'GET',
                    timeout: 5000
                });
        ...
        this.results.penetration.tests.xss_vulnerabilities = {
            status: vulnerable ? 'failed' : 'passed',
            message: vulnerable ? 'XSS vulnerabilities detected' : 'No XSS vulnerabilities found',
            payloads: responses
        };
    }
    ...
    async testInformationDisclosure(baseUrl) {
            ...
            try {
                ...
                if (response.statusCode === 200) {
                    disclosedFiles.push({
                        ...
                        sensitive: ['.env', '.git/config', 'web.config', '.htaccess'].includes(file)
                    });
                }
            } catch (error) {
        ...
        this.results.penetration.tests.information_disclosure = {
            ...
            message: sensitiveDisclosed.length > 0 ? 
    ...
    async testTLSVersion(hostname, version) {
        return new Promise((resolve, reject) => {
            const options = {
                host: hostname,
                port: 443,
                secureProtocol: version === 'TLSv1.3' ? 'TLSv1_3_method' : 
                               version === 'TLSv1.2' ? 'TLSv1_2_method' :
                               'TLSv1_method',
                rejectUnauthorized: false
            };
    ...
    async testHTTPSRedirect(hostname) {
        try {
            ...
            return {
                status: response.statusCode >= 300 && response.statusCode < 400 ? 'passed' : 'failed',
                statusCode: response.statusCode,
    ...
    async generateReport() {
        const report = {
            summary: {
                ...
                score: Math.round((this.results.overall.passed / (this.results.overall.passed + this.results.overall.failed)) * 100) || 0
            },
        ...
        const summaryPath = path.join(process.cwd(), 'security-validation-summary.md');
  ...
  node security-validation-suite.js --domain example.com --cloudfront d123456.cloudfront.net --verbose
    ...
    try {
        ...
        if (options.cloudfrontDomain) {
            ...
            await validator.validateSecurityHeaders(`https://${options.cloudfrontDomain}`);
        }
