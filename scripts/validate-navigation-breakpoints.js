...
class NavigationValidator {
  ...
  validateHeaderComponent() {
    console.log(
      '🔍 Validating Header component navigation implementation...\n'
    );
    ...
    const hamburgerHiddenRegex =
      /className=['"][^'"]*md:hidden[^'"]*['"][^>]*aria-label=['"]Toggle mobile menu['"]|aria-label=['"]Toggle mobile menu['"][^>]*className=['"][^'"]*md:hidden[^'"]*['"]/;
    this.validationResults.hamburgerHidden =
      hamburgerHiddenRegex.test(headerContent);
    console.log(
      `   ${this.validationResults.hamburgerHidden ? '✅' : '❌'} Hamburger button has md:hidden class`
    );

    // Test 2: Desktop navigation visible (hidden md:flex)
    console.log('\n🖥️  Test 2: Desktop navigation visible');
    const desktopNavRegex =
      /className=['"][^'"]*hidden md:flex[^'"]*['"][^>]*role=['"]navigation['"]|role=['"]navigation['"][^>]*className=['"][^'"]*hidden md:flex[^'"]*['"]/;
    this.validationResults.desktopNavVisible =
      desktopNavRegex.test(headerContent);
    console.log(
      `   ${this.validationResults.desktopNavVisible ? '✅' : '❌'} Desktop navigation has hidden md:flex classes`
    );

    // Test 3: Desktop CTA button visible (hidden md:flex)
    console.log('\n🎯 Test 3: Desktop CTA button visible');
    const desktopCTARegex =
      /{\/\* Desktop CTA Button \*\/}[\s\S]*?<div className=['"][^'"]*hidden md:flex[^'"]*['"][\s\S]*?Get Started/;
    this.validationResults.desktopCTAVisible =
      desktopCTARegex.test(headerContent);
    console.log(
      `   ${this.validationResults.desktopCTAVisible ? '✅' : '❌'} Desktop CTA button has hidden md:flex classes`
    );
    ...
    this.validationResults.correctBreakpoint =
      breakpointMatches && breakpointMatches.length >= 2;
    console.log(
      `   ${this.validationResults.correctBreakpoint ? '✅' : '❌'} Uses md breakpoint (768px) consistently`
    );

    // Test 5: Accessibility attributes
    console.log('\n♿ Test 5: Accessibility attributes');
    const accessibilityRegex =
      /aria-label=['"]Toggle mobile menu['"][\s\S]*?aria-expanded=\{isMobileMenuOpen\}/;
    this.validationResults.accessibilityAttributes =
      accessibilityRegex.test(headerContent);
    console.log(
      `   ${this.validationResults.accessibilityAttributes ? '✅' : '❌'} Proper accessibility attributes present`
    );

    // Overall validation
    const allTestsPassed = Object.values(this.validationResults)
      .slice(0, -1)
  ...
  generateReport() {
    ...
    const report = {
      ...
      summary: {
        totalTests: 5,
        passedTests: Object.values(this.validationResults)
          .slice(0, -1)
          .filter(r => r === true).length,
        failedTests: Object.values(this.validationResults)
          .slice(0, -1)
  ...
  generateRecommendations() {
    ...
    if (!this.validationResults.hamburgerHidden) {
      recommendations.push(
        'Add md:hidden class to hamburger button to hide on desktop'
      );
    }
    ...
    if (!this.validationResults.correctBreakpoint) {
      recommendations.push(
        'Ensure consistent use of md breakpoint (768px) for responsive behavior'
      );
    }

    if (!this.validationResults.accessibilityAttributes) {
      recommendations.push(
        'Add proper aria-label and aria-expanded attributes to hamburger button'
      );
    }

    if (recommendations.length === 0) {
      recommendations.push(
        'All navigation breakpoint requirements are properly implemented'
      );
    }
  ...
  printSummary(report) {
    ...
    console.log(
      `Status: ${report.summary.overallStatus === 'PASSED' ? '✅ PASSED' : '❌ FAILED'}`
    );
    console.log(
    ...
    console.log('\n📋 Test Results:');
    console.log(
      `   Hamburger Hidden on Desktop: ${this.validationResults.hamburgerHidden ? '✅' : '❌'}`
    );
    console.log(
      `   Desktop Navigation Visible: ${this.validationResults.desktopNavVisible ? '✅' : '❌'}`
    );
    console.log(
      `   Desktop CTA Visible: ${this.validationResults.desktopCTAVisible ? '✅' : '❌'}`
    );
    console.log(
      `   Correct Breakpoint Usage: ${this.validationResults.correctBreakpoint ? '✅' : '❌'}`
    );
    console.log(
      `   Accessibility Attributes: ${this.validationResults.accessibilityAttributes ? '✅' : '❌'}`
    );
