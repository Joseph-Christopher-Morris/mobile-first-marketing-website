...
class TLSValidationTester {
    ...
    validateCloudFrontTLS(results, testResult) {
        ...
        tls12Check.status = tls12Check.actual === tls12Check.expected ? 'passed' : 'failed';
        ...
        const weakTLSCheck = {
            ...
            expected: false,
            actual: results.tlsVersions['TLSv1.0']?.supported || results.tlsVersions['TLSv1.1']?.supported || false,
            status: 'unknown'
        };
        weakTLSCheck.status = weakTLSCheck.actual === weakTLSCheck.expected ? 'passed' : 'failed';
        ...
        scoreCheck.status = scoreCheck.actual === scoreCheck.expected ? 'passed' : 'warning';
        testResult.checks.push(scoreCheck);

        console.log(`  TLS 1.2 Support: ${tls12Check.status === 'passed' ? '✅' : '❌'}`);
        console.log(`  Weak TLS Disabled: ${weakTLSCheck.status === 'passed' ? '✅' : '❌'}`);
        console.log(`  Security Score: ${results.securityScore}/100 ${scoreCheck.status === 'passed' ? '✅' : '⚠️'}`);
    }
    ...
    validateSecureDomainTLS(results, testResult) {
        ...
        tls13Check.status = tls13Check.actual === tls13Check.expected ? 'passed' : 'warning';
        ...
        pfsCheck.status = pfsCheck.actual === pfsCheck.expected ? 'passed' : 'warning';
        ...
        weakCipherCheck.status = weakCipherCheck.actual === weakCipherCheck.expected ? 'passed' : 'warning';
        testResult.checks.push(weakCipherCheck);

        console.log(`  TLS 1.3 Support: ${tls13Check.status === 'passed' ? '✅' : '⚠️'}`);
        console.log(`  Perfect Forward Secrecy: ${pfsCheck.status === 'passed' ? '✅' : '⚠️'}`);
        console.log(`  Weak Ciphers: ${results.weakCiphers.length} ${weakCipherCheck.status === 'passed' ? '✅' : '⚠️'}`);
    }
    ...
    testTLSVersionLogic(testResult) {
        ...
        const hasWeakTLSWarning = validator.results.recommendations.some(rec => 
            rec.message.includes('TLSv1.0') || rec.message.includes('TLSv1.1')
        );
        ...
        console.log(`  TLS Version Logic: ${logicCheck.status === 'passed' ? '✅' : '❌'}`);
    }
    ...
    testCipherStrengthLogic(testResult) {
        ...
        const hasWeakCipher = testCiphers.some(cipher => cipher.strength === 'weak');
        const hasStrongCipher = testCiphers.some(cipher => cipher.strength === 'strong');
        ...
        console.log(`  Cipher Strength Logic: ${cipherLogicCheck.status === 'passed' ? '✅' : '❌'}`);
    }
    ...
    testSecurityScoringLogic(testResult) {
        ...
        console.log(`  Security Scoring: ${validator.results.securityScore}/100 ${scoreCheck.status === 'passed' ? '✅' : '❌'}`);
    }
    ...
    saveTestResults() {
        ...
        const resultsFile = path.join(resultsDir, 'tls-validation-test-results.json');
