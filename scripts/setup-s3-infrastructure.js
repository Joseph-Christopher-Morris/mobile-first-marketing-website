...
class S3InfrastructureSetup {
  constructor() {
    ...
    this.bucketName =
      process.env.S3_BUCKET_NAME || `mobile-marketing-site-${Date.now()}`;
  ...
  async blockPublicAccess() {
    ...
    await this.s3Client.send(
      new PutBucketPublicAccessBlockCommand(publicAccessBlockParams)
    );
  ...
  async setBucketPolicy() {
    ...
    console.log(
      'ℹ️  Policy will be updated when CloudFront distribution is created'
    );
  }
