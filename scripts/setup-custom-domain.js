...
class CustomDomainSetup {
  ...
  async verifyCertificate() {
    ...
    if (!this.certificateArn) {
      console.log(
        'ℹ️  No SSL certificate ARN provided, checking distribution configuration...'
      );
      ...
      if (viewerCert && viewerCert.ACMCertificateArn) {
        ...
        console.log(
          `✅ Found certificate in distribution: ${this.certificateArn}`
        );
      } else {
        throw new Error(
          'No SSL certificate found. Please run setup-ssl-certificate.js first.'
        );
      }
    }
    ...
    try {
      ...
      if (certificate.SubjectAlternativeNames) {
        console.log(
          `   Alternative names: ${certificate.SubjectAlternativeNames.join(', ')}`
        );
      }

      if (certificate.Status !== 'ISSUED') {
        throw new Error(
          `Certificate is not ready. Status: ${certificate.Status}`
        );
      }
      ...
      const uncoveredDomains = this.domains.filter(
        domain => !certDomains.includes(domain)
      );

      if (uncoveredDomains.length > 0) {
        console.warn(
          `⚠️  Certificate does not cover: ${uncoveredDomains.join(', ')}`
        );
  ...
  async updateDistributionAliases() {
    ...
    try {
      ...
      const aliasesMatch =
        this.domains.every(domain => currentAliases.includes(domain)) &&
      ...
      if (
        updatedConfig.DefaultCacheBehavior.ViewerProtocolPolicy !==
        'redirect-to-https'
      ) {
        updatedConfig.DefaultCacheBehavior.ViewerProtocolPolicy =
          'redirect-to-https';
  ...
  async findHostedZone() {
    ...
    try {
      ...
      if (bestMatch) {
        ...
        console.log(
          'ℹ️  No Route 53 hosted zone found for automatic DNS configuration'
        );
        return null;
      }
    } catch (error) {
  ...
  async createDNSRecords() {
    ...
    if (!this.hostedZoneId) {
      console.log(
        'ℹ️  No hosted zone available, skipping automatic DNS configuration'
      );
      return false;
    }
    ...
    try {
      ...
      const existingRecords = await this.route53Client.send(
      ...
      for (const domain of this.domains) {
        ...
        if (existingRecord) {
          console.log(
            `ℹ️  DNS record for ${domain} already exists, updating...`
          );
  ...
  generateManualDNSInstructions() {
    ...
    this.domains.forEach((domain, index) => {
      ...
      console.log(
        `   Record Value: ${this.distributionDomainName} (ALIAS/CNAME)`
      );
    ...
    console.log(
      '3. If ALIAS is not supported, use CNAME for www and A record for apex domain'
    );
    ...
    console.log(
      '• GoDaddy: Use CNAME for www, A record with CloudFront IP for apex'
    );
    console.log('• Namecheap: Use ALIAS record if available, otherwise CNAME');
    console.log(
      '• Route 53: ALIAS records are recommended (automatic configuration attempted)'
    );
  ...
  async run() {
    try {
      ...
      console.log(`   • HTTPS Redirect: Enabled`);
      console.log(
        `   • DNS Configuration: ${dnsConfigured ? 'Automatic (Route 53)' : 'Manual Required'}`
      );

      if (dnsConfigured) {
        console.log('\n⏳ Next steps:');
        console.log(
          '1. Wait for CloudFront distribution update (15-20 minutes)'
        );
        ...
        console.log(
          '2. Wait for CloudFront distribution update (15-20 minutes)'
        );
      ...
      console.log(
        `   curl -I http://${this.customDomain} # Should redirect to HTTPS`
      );
      ...
      console.error(
        '1. Verify AWS credentials have CloudFront and Route 53 permissions'
      );
      console.error('2. Ensure SSL certificate is issued and valid');
      console.error(
        '3. Check that CloudFront distribution exists and is deployed'
      );
