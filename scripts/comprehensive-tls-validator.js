...
class ComprehensiveTLSValidator {
  ...
  async testSecureTLSVersions() {
    ...
    for (const tlsVersion of secureTLSVersions) {
      try {
        ...
        console.log(
          `  ${tlsVersion.name}: ${this.results.tlsVersionSupport[tlsVersion.name].status}`
        );
        ...
        if (result.supported) {
          ...
          this.results.recommendations.push(
            `Enable ${tlsVersion.name} support - this is required for security compliance`
          );
        } else {
          this.results.testDetails.warnings++;
          this.results.recommendations.push(
            `Consider enabling ${tlsVersion.name} support for enhanced security`
          );
        }
      } catch (error) {
        this.results.tlsVersionSupport[tlsVersion.name] = {
      ...
      try {
        ...
        if (!result.supported) {
          ...
          if (tlsVersion.critical) {
            this.results.testDetails.failedTests++;
            this.results.recommendations.push(
              `CRITICAL: Disable ${tlsVersion.name} support immediately - this is a security vulnerability`
            );
          } else {
            this.results.testDetails.warnings++;
            this.results.recommendations.push(
  ...
  async analyzeCipherSuites() {
    ...
    for (const version of tlsVersionsToTest) {
      try {
        ...
        if (result.supported && result.cipher) {
          ...
          console.log(
            `    Strength: ${cipherInfo.strong ? '✅ STRONG' : '⚠️  WEAK'}`
          );
          console.log(`    PFS: ${cipherInfo.pfs ? '✅ YES' : '❌ NO'}`);
          console.log(
            `    Encryption: ${cipherInfo.strongEncryption ? '✅ STRONG' : '⚠️  WEAK'}`
          );
        }
      } catch (error) {
    ...
    if (
      cipherAnalysis.pfsSupported &&
      cipherAnalysis.strongEncryption &&
      cipherAnalysis.weakCiphers.length === 0
    ) {
    ...
    console.log(
      `  Perfect Forward Secrecy: ${cipherAnalysis.pfsSupported ? '✅ YES' : '❌ NO'}`
    );
    console.log(
      `  Strong Encryption: ${cipherAnalysis.strongEncryption ? '✅ YES' : '❌ NO'}`
    );
  ...
  async validatePerfectForwardSecrecy() {
    ...
    for (const cipherInfo of this.results.cipherSuiteAnalysis
      .negotiatedCiphers) {
    ...
    console.log(
      `  Ciphers without PFS: ${pfsResults.ciphersWithoutPFS.length}`
    );
    ...
    if (pfsResults.ciphersWithPFS.length > 0) {
      ...
      pfsResults.ciphersWithPFS.forEach(cipher => {
        console.log(
          `    - ${cipher.cipher} (${cipher.tlsVersion}, ${cipher.keyExchange})`
        );
      });
    }
    ...
    if (pfsResults.ciphersWithoutPFS.length > 0) {
      ...
      pfsResults.recommendation =
        'Configure server to prefer PFS cipher suites';
    }
    ...
    if (pfsResults.supported) {
      ...
      this.results.testDetails.failedTests++;
      this.results.recommendations.push(
        'Enable Perfect Forward Secrecy (PFS) - use ECDHE or DHE key exchange'
      );
    }
  }
  ...
  generateSecurityAssessment() {
    ...
    if (cipherLevel === 'HIGH') assessment.categories.cipherSuites.score = 30;
      ...
      assessment.categories.cipherSuites.score = 20;
    else if (cipherLevel === 'LOW')
      assessment.categories.cipherSuites.score = 10;
    ...
    assessment.overallScore = Object.values(assessment.categories).reduce(
    ...
    console.log(
      `  Overall Score: ${assessment.overallScore}/${assessment.maxScore}`
    );
    console.log(`  Grade: ${assessment.grade}`);
    console.log(`  Status: ${assessment.status}`);
    console.log(
      `  TLS Versions: ${assessment.categories.tlsVersions.score}/${assessment.categories.tlsVersions.maxScore}`
    );
    console.log(
      `  Cipher Suites: ${assessment.categories.cipherSuites.score}/${assessment.categories.cipherSuites.maxScore}`
    );
    console.log(
      `  Perfect Forward Secrecy: ${assessment.categories.perfectForwardSecrecy.score}/${assessment.categories.perfectForwardSecrecy.maxScore}`
    );
    console.log(
      `  Compliance: ${assessment.categories.compliance.score}/${assessment.categories.compliance.maxScore}`
    );
  }
  ...
  checkCompliance() {
    ...
    const pciCompliant =
      this.results.tlsVersionSupport['TLS 1.2']?.supported &&
      !this.results.tlsVersionSupport['TLS 1.0']?.supported &&
      !this.results.tlsVersionSupport['TLS 1.1']?.supported &&
    ...
    console.log(
      `  PCI DSS: ${compliance['PCI DSS'] ? '✅ COMPLIANT' : '❌ NON-COMPLIANT'}`
    );
    console.log(
      `  NIST: ${compliance['NIST'] ? '✅ COMPLIANT' : '❌ NON-COMPLIANT'}`
    );
    console.log(
      `  OWASP: ${compliance['OWASP'] ? '✅ COMPLIANT' : '❌ NON-COMPLIANT'}`
    );
  }
  ...
  generateRecommendations() {
    ...
    if (this.results.recommendations.length === 0) {
      ...
      sortedRecommendations.forEach((recommendation, index) => {
        const priority = recommendation.includes('CRITICAL')
          ? '🚨 CRITICAL'
          : recommendation.includes('Enable TLS')
            ? '⚠️  HIGH'
            : '💡 MEDIUM';
  ...
  saveResults(filename) {
    const outputFile =
      filename ||
      `comprehensive-tls-validation-${this.domain}-${Date.now()}.json`;
  ...
  async validate() {
    ...
    try {
      ...
      console.log(
        `📊 Final Score: ${this.results.securityAssessment.overall?.overallScore || 0}/100 (${this.results.securityAssessment.overall?.grade || 'F'})`
      );
...
Examples:
  ...
  validator
    ...
    .then(results => {
      ...
      const hasCriticalIssues = results.recommendations.some(r =>
        r.includes('CRITICAL')
      );
