...
class BuildImageInclusionFixer {
    ...
    try {
      if (fs.existsSync(this.buildDir)) {
        ...
        try {
          ...
          execSync(`powershell -Command "Remove-Item -Recurse -Force '${this.buildDir}'"`, { 
            stdio: 'pipe',
            timeout: 30000 
          });
          ...
          try {
            ...
            try {
              ...
              console.warn('   ⚠️  Could not fully clean build directory, continuing...');
              console.warn(`   Errors: ${error1.message}, ${error2.message}, ${error3.message}`);
            }
          }
        }
      }
      ...
      if (cleanupSuccess) {
        ...
        console.log('   ⚠️  Build directory partially cleaned, proceeding with build');
      }
  ...
  async verifyImageInclusion() {
    ...
    try {
      ...
      if (sizeMismatches.length > 0) {
        console.warn(`⚠️  ${sizeMismatches.length} images have size mismatches:`);
        sizeMismatches.forEach(mismatch => {
          console.warn(`   - ${mismatch.relativePath}: ${mismatch.sourceSize} → ${mismatch.buildSize} bytes`);
        });
      }
      
      if (extraImages.length > 0) {
        console.log(`ℹ️  ${extraImages.length} extra images found in build (not in source)`);
      }
  ...
  async testBlogImage() {
    ...
    try {
      const sourcePath = path.join(process.cwd(), this.publicDir, blogImagePath);
      ...
      if (sourceExists && buildExists) {
        console.log(`   Sizes match: ${blogImageTest.sizesMatch ? '✅' : '❌'} (${sourceSize} vs ${buildSize} bytes)`);
      }
  ...
  generateReport() {
    ...
    if (this.results.verification.sourceCount !== undefined) {
      ...
      if (this.results.verification.sizeMismatchCount > 0) {
        console.log(`   Size mismatches: ${this.results.verification.sizeMismatchCount}`);
      }
    }
    ...
    const reportPath = path.join(process.cwd(), 'build-image-inclusion-fix-report.json');
...
if (require.main === module) {
  ...
  fixer.run()
    ...
    .catch((error) => {
      console.error('\n❌ Build image inclusion fix process failed:', error.message);
      process.exit(1);
    });
}
