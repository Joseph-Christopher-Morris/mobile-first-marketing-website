...
class ImagePerformanceTester {
  ...
  async runPerformanceTests() {
    ...
    try {
      ...
      console.log(
        `📊 Results saved to: image-performance-results-${Date.now()}.json`
      );
    } catch (error) {
  ...
  async testImageLoadTimes() {
    ...
    for (const imagePath of this.testImages) {
      ...
      console.log(
        `    Status: ${loadTest.success ? '✅ Success' : '❌ Failed'}`
      );
    }
  }
  ...
  getLoadTimeRecommendations(loadTest) {
    ...
    if (loadTest.loadTime > 3000) {
      recommendations.push(
        'Load time exceeds 3 seconds - consider optimization'
      );
    }
  ...
  async testCacheEfficiency() {
    ...
    for (const imagePath of this.testImages) {
      ...
      const improvement =
        firstTime > 0 ? ((firstTime - secondTime) / firstTime) * 100 : 0;
    ...
    for (const viewport of responsiveTest.viewportSizes) {
      ...
      if (viewport.width <= 375) {
        ...
        result.recommendations.push(
          'Provide high-resolution images for large screens'
        );
      }
  ...
  calculatePerformanceImpact(viewportWidth) {
    ...
    if (viewportWidth <= 375)
      return 'High - mobile users sensitive to load times';
    if (viewportWidth <= 768)
      return 'Medium - balance between quality and speed';
  ...
  async generateRecommendations() {
    ...
    if (averageLoadTime > 2000) {
      recommendations.push(
        'Average load time exceeds 2 seconds - implement optimization'
      );
    }
    ...
    if (totalSize > 500000) {
      recommendations.push(
        'Total image size exceeds 500KB - consider compression'
      );
    }
  ...
  calculateSummary() {
    ...
    const successfulTests = Object.values(imageTests).filter(
      test => test.success
    );
    ...
    this.testResults.summary = {
      ...
      averageLoadTime:
        successfulTests.length > 0
          ? Math.round(
              successfulTests.reduce((sum, test) => sum + test.loadTime, 0) /
                successfulTests.length
            )
          : 0,
      totalDataTransferred: successfulTests.reduce(
        (sum, test) => sum + test.fileSize,
        0
      ),
  ...
  calculateCompressionSavings() {
    ...
    const jpegImages = Object.entries(this.testResults.imageTests).filter(
      ([path, test]) => path.endsWith('.jpg') && test.success
    );
...
### Load Time Analysis
${Object.entries(this.testResults.imageTests)
  .map(
    ([path, test]) => `
**${path}**
