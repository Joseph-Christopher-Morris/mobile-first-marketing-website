...
class HttpsRedirectValidationTester {
    ...
    async testKnownGoodDomain() {
        ...
        try {
            ...
            console.log(`✅ GitHub test completed: ${results.summary.successRate} success rate`);
        ...
        try {
            ...
            if (configExists) {
                ...
                if (domain) {
                    ...
                    console.log(`✅ CloudFront test completed: ${results.summary.successRate} success rate`);
                } else {
    ...
    async testValidationFunctionality() {
        ...
        try {
            ...
            const hstsTests = [
                {
                    ...
                    expected: { maxAge: 31536000, includeSubDomains: true, preload: true }
                },
                {
            ...
            for (const test of hstsTests) {
                ...
                this.addTestResult('HSTS Header Parsing', isCorrect ? 'PASSED' : 'FAILED', {
                    input: test.header,
            ...
            for (const test of cookieTests) {
                ...
                const isCorrect = analysis.secure === test.expectedSecure && 
                                analysis.httpOnly === test.expectedHttpOnly;
                
                this.addTestResult('Cookie Analysis', isCorrect ? 'PASSED' : 'FAILED', {
                    input: test.cookie,
                    expected: { secure: test.expectedSecure, httpOnly: test.expectedHttpOnly },
            ...
            for (const test of redirectTests) {
                ...
                this.addTestResult('Redirect Type Identification', isCorrect ? 'PASSED' : 'FAILED', {
...
async function main() {
    ...
    try {
        ...
        const outputPath = path.join(__dirname, '..', 'config', `https-redirect-test-results-${timestamp}.json`);
