...
class CachingStrategyOptimizer {
  ...
  async updateS3CacheHeaders() {
    ...
    if (!fs.existsSync(this.buildDir)) {
      throw new Error(
        `Build directory '${this.buildDir}' not found. Run 'npm run build' first.`
      );
    }
    ...
    console.log(
      `📋 Found ${filesToUpdate.length} files to update with optimized cache headers`
    );
    ...
    Object.entries(cacheStats).forEach(([strategy, stats]) => {
      if (stats.count > 0) {
        ...
        const description = strategyInfo
          ? strategyInfo.description
          : 'Default caching';
        console.log(
          `   ${strategy}: ${stats.count} files (${this.formatBytes(stats.totalSize)}) - ${description}`
        );
      }
    });
    console.log('');
        ...
        batch.map(async file => {
          try {
            ...
            if (
              ...
              console.log(
                `   Updated ${updatedCount}/${filesToUpdate.length} files`
              );
            }
          } catch (error) {
  ...
  async testCacheBehavior() {
    ...
    try {
      ...
      console.log(
        `📋 Testing cache headers for ${listResult.Contents.length} files...`
      );
      ...
      console.log(
        `   Success Rate: ${((testResults.passed / testResults.totalFiles) * 100).toFixed(1)}%`
      );
      ...
      Object.entries(testResults.strategies).forEach(([strategy, results]) => {
        if (results.files.length > 0) {
          const successRate = (
            (results.correct / results.files.length) *
            100
          ).toFixed(1);
          console.log(
            `   ${strategy}: ${results.correct}/${results.files.length} correct (${successRate}%)`
          );
        }
      });
      ...
      if (testResults.issues.length > 0) {
        ...
        testResults.issues.slice(0, 10).forEach(issue => {
          if (issue.error) {
            ...
            console.log(
              `   ${issue.file}: Expected "${issue.expected}", got "${issue.actual}"`
            );
          }
        });
        ...
        if (testResults.issues.length > 10) {
          console.log(
            `   ... and ${testResults.issues.length - 10} more issues`
          );
        }
      }
      ...
      const resultsPath = path.join(
        process.cwd(),
        'cache-behavior-test-results.json'
      );
  ...
  async testCacheInvalidation() {
    ...
    try {
      ...
      const testPaths = ['/index.html', '/images/*', '/*.html'];

      console.log(
        `📋 Creating test invalidation for ${testPaths.length} paths...`
      );
        ...
        try {
          ...
          if (status === 'Completed') {
            ...
            const testPath = path.join(
              process.cwd(),
              'cache-invalidation-test-results.json'
            );
            fs.writeFileSync(
              testPath,
              JSON.stringify(invalidationTest, null, 2)
            );
          ...
          console.error(
            `   Error checking invalidation status: ${error.message}`
          );
        }
      }
  ...
  generateCachingReport(cacheStats, behaviorTest, invalidationTest) {
    ...
    const report = {
      ...
      implementation: {
        ...
        totalFiles: Object.values(cacheStats).reduce(
          (sum, stats) => sum + stats.count,
          0
        ),
        totalSize: Object.values(cacheStats).reduce(
      ...
      testing: {
        behaviorTest: {
          ...
          successRate:
            ((behaviorTest.passed / behaviorTest.totalFiles) * 100).toFixed(1) +
            '%',
    ...
    const reportPath = path.join(
      process.cwd(),
      'caching-strategy-optimization-report.json'
    );
    ...
    const summaryPath = path.join(
      process.cwd(),
      'caching-strategy-optimization-summary.md'
    );
...
`;
  ...
  async run() {
    ...
    try {
      ...
      const report = this.generateCachingReport(
        cacheStats,
        behaviorTest,
        invalidationTest
      );
      ...
      console.log('📋 Summary:');
      console.log(
        `   ✅ Images: Long-term caching (${this.cachingStrategies.images.maxAge}s)`
      );
      console.log(
        `   ✅ HTML: Short-term caching (${this.cachingStrategies.html.maxAge}s)`
      );
      console.log(
        `   ✅ Cache behavior tested: ${behaviorTest.successRate} success rate`
      );
      console.log(
        `   ✅ Invalidation tested: ${invalidationTest.effective ? 'Effective' : 'In Progress'}`
      );
      console.log('');
      ...
      console.error(
        '\n❌ Caching strategy optimization failed:',
        error.message
      );
...
if (require.main === module) {
  ...
  optimizer
    ...
    .catch(error => {
      console.error(
        '\n❌ Caching strategy optimization process failed:',
        error.message
      );
      process.exit(1);
    });
