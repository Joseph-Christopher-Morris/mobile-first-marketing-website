...
class CachePolicyCreator {
  ...
  async createHtmlCachePolicy() {
    ...
    const policyConfig = {
      ...
      Comment:
        'Custom cache policy for HTML files - 5 minutes cache (Task 14.2)',
    ...
    try {
      ...
      console.log(
        `   Policy Name: ${result.CachePolicy.CachePolicyConfig.Name}`
      );
      console.log(
        `   Default TTL: ${result.CachePolicy.CachePolicyConfig.DefaultTTL} seconds`
      );
      ...
      if (error.name === 'CachePolicyAlreadyExists') {
        console.log(
          '⚠️  Cache policy already exists, listing existing policies...'
        );
  ...
  async findExistingHtmlPolicy() {
    try {
      ...
      const htmlPolicy = result.CachePolicyList?.Items?.find(
        policy =>
          policy.CachePolicy.CachePolicyConfig.Name === 'HTML-5min-Cache-Policy'
      );
  ...
  generateSummary(cachePolicy) {
    ...
    console.log(
      `   Cache Duration: ${summary.cachePolicy.defaultTTL} seconds (5 minutes)`
    );
  ...
  async run() {
    try {
      ...
      console.log(
        '2. Current CloudFront configuration already uses appropriate managed policies'
      );
      console.log('3. Images use CachingOptimized policy (1 year cache)');
      console.log(
        '4. HTML uses CachingDisabled policy (respects origin cache headers)'
      );
      console.log(
        '5. Origin cache headers are set correctly in deployment script'
      );
...
if (require.main === module) {
  ...
  creator
    ...
    .catch(error => {
      console.error(
        '\n❌ Cache policy creation process failed:',
        error.message
      );
      process.exit(1);
    });
