...
class TLSVersionTester {
  ...
  async testTLS12Support() {
    ...
    try {
      ...
      console.log(
        `  TLS 1.2: ${this.results.tlsVersionTests['TLS 1.2'].status}`
      );
      if (result.supported) {
      ...
      if (result.supported) {
        ...
        this.results.recommendations.push(
          'CRITICAL: Enable TLS 1.2 support - required for security compliance'
        );
      }
    } catch (error) {
      this.results.tlsVersionTests['TLS 1.2'] = {
  ...
  async testTLS13Support() {
    ...
    try {
      ...
      console.log(
        `  TLS 1.3: ${this.results.tlsVersionTests['TLS 1.3'].status}`
      );
      if (result.supported) {
        console.log(`    Negotiated Protocol: ${result.negotiatedProtocol}`);
        console.log(`    Cipher: ${result.cipher?.name || 'Unknown'}`);
      ...
      if (result.supported) {
        ...
        this.results.recommendations.push(
          'Consider enabling TLS 1.3 support for enhanced security and performance'
        );
      }
    } catch (error) {
      this.results.tlsVersionTests['TLS 1.3'] = {
  ...
  async testWeakTLSVersionsDisabled() {
    ...
    for (const weakVersion of weakVersions) {
      try {
        const result = await this.testSpecificTLSVersion(
          weakVersion.version,
          weakVersion.options
        );
        ...
        console.log(
          `  ${weakVersion.name}: ${this.results.tlsVersionTests[weakVersion.name].status}`
        );
        ...
        if (!result.supported) {
          ...
          this.results.summary.failedTests++;
          this.results.recommendations.push(
            `CRITICAL: Disable ${weakVersion.name} support - this is a security vulnerability`
          );
        }
      } catch (error) {
  ...
  async testTLSVersionNegotiation() {
    ...
    for (const test of negotiationTests) {
      try {
        ...
        console.log(
          `  ${test.name}: ${this.results.negotiationTests[test.name].status}`
        );
        ...
        if (negotiationSuccess) {
          ...
          this.results.summary.warnings++;
          this.results.recommendations.push(
            `Review TLS version negotiation: ${negotiationNote}`
          );
        }
      } catch (error) {
        this.results.negotiationTests[test.name] = {
  ...
  determineComplianceStatus() {
    ...
    const hasWeakTLS =
      this.results.tlsVersionTests['TLS 1.0']?.supported ||
  ...
  generateSummary() {
    ...
    console.log(`\n🔒 TLS Version Support:`);
    Object.entries(this.results.tlsVersionTests).forEach(
      ([version, result]) => {
    ...
    if (Object.keys(this.results.negotiationTests).length > 0) {
      console.log(`\n🤝 Negotiation Tests:`);
      Object.entries(this.results.negotiationTests).forEach(
        ([test, result]) => {
  ...
  saveResults(filename) {
    const outputFile =
      filename || `tls-version-support-test-${this.domain}-${Date.now()}.json`;
