...
class RollbackManager {
    ...
    try {
      ...
      for (const file of currentFiles) {
        ...
        if (backedUpCount % 10 === 0) {
          console.log(
            `   Backed up ${backedUpCount}/${currentFiles.length} files`
          );
        }
      }
  ...
  async getCurrentFiles() {
    ...
    do {
      ...
      if (result.Contents) {
        ...
        const currentFiles = result.Contents.filter(
          file =>
  ...
  async calculateBackupIntegrity(files) {
    ...
    const integrityData = JSON.stringify(
      fileHashes.sort((a, b) => a.key.localeCompare(b.key))
    );
  ...
  async listBackups() {
    ...
    try {
      ...
      for (const prefix of result.CommonPrefixes) {
        ...
        try {
          const metadataResult = await this.s3Client.send(
            new GetObjectCommand({
              Bucket: this.bucketName,
              Key: metadataKey,
            })
  ...
  async rollbackToBackup(backupId) {
    ...
    try {
      ...
      console.log(
        `   Created: ${new Date(backupMetadata.timestamp).toLocaleString()}`
      );
      ...
      if (backupMetadata.git && backupMetadata.git.shortCommit) {
        console.log(
          `   Git Commit: ${backupMetadata.git.shortCommit} - ${backupMetadata.git.message}`
        );
      }
      console.log('');
  ...
  async getBackupFiles(backupId) {
    ...
    do {
      ...
      if (result.Contents) {
        ...
        const backupFiles = result.Contents.filter(
          file => !file.Key.endsWith(this.metadataFile)
        );
  ...
  async clearCurrentDeployment() {
    ...
    for (let i = 0; i < currentFiles.length; i += batchSize) {
      ...
      console.log(
        `   Deleted ${Math.min(i + batchSize, currentFiles.length)}/${currentFiles.length} files`
      );
    }
  ...
  async restoreFilesFromBackup(backupId, backupFiles) {
    ...
    for (const file of backupFiles) {
      ...
      const targetKey = file.Key.replace(
  ...
  async emergencyRollback() {
    ...
    const lastGoodBackup = backups.find(
      backup => backup.type !== 'pre-rollback' && backup.fileCount > 0
    );
    ...
    console.log(
      `   Created: ${new Date(lastGoodBackup.timestamp).toLocaleString()}`
    );
