...
function checkPrerequisites() {
  ...
  if (!fs.existsSync(CONFIG.buildDir)) {
    console.error(
      `Build directory '${CONFIG.buildDir}' not found. Run 'npm run build' first.`
    );
    process.exit(1);
  }
...
function createCloudFrontDistribution() {
  ...
  log(
    'CloudFront distribution created! It may take 15-20 minutes to deploy globally.'
  );
}
...
function main() {
  ...
  log(
    'Run with --cloudfront flag to create CloudFront distribution automatically'
  );
