...
class DeploymentImageVerification {
  ...
  validateConfiguration() {
    ...
    if (!this.distributionId) {
      console.warn(
        '⚠️  CLOUDFRONT_DISTRIBUTION_ID not set, skipping CloudFront tests'
      );
    }
  ...
  async verifyS3Upload() {
    ...
    try {
      ...
      for (const image of buildImages) {
        try {
          ...
          if (actualMimeType !== expectedMimeType) {
            console.warn(
              `⚠️  MIME type mismatch for ${image.s3Key}: expected ${expectedMimeType}, got ${actualMimeType}`
            );
          }
        } catch (error) {
      ...
      console.log(
        `   Correct MIME types: ${correctMimeTypes}/${mimeTypeResults.length}`
      );

      if (uploadedCount < buildImages.length) {
        console.error(
          `❌ ${buildImages.length - uploadedCount} images missing from S3`
        );
        return false;
      }
      ...
      if (correctMimeTypes < mimeTypeResults.length) {
        console.warn(
          `⚠️  ${mimeTypeResults.length - correctMimeTypes} images have incorrect MIME types`
        );
      }
  ...
  async testImageAccessibility() {
    ...
    try {
      ...
      if (!domain) {
        console.warn(
          '⚠️  CloudFront domain not available, skipping accessibility tests'
        );
      ...
      console.log(
        `   Testing ${imagesToTest.length} images via https://${domain}`
      );
      ...
      for (const image of imagesToTest) {
        ...
        if (result.success) {
          console.log(
            `   ✅ ${image.s3Key} - ${result.statusCode} (${result.contentLength} bytes)`
          );
        } else {
          console.error(
            `   ❌ ${image.s3Key} - ${result.error || result.statusCode}`
          );
        }
      }

      const successfulTests = accessibilityResults.filter(
        r => r.success
      ).length;
      ...
      console.log(
        `   Accessible: ${successfulTests}/${imagesToTest.length} images`
      );

      if (successfulTests < imagesToTest.length) {
        console.error(
          `❌ ${imagesToTest.length - successfulTests} images not accessible via CloudFront`
        );
        return false;
      }
      ...
      console.error(
        '❌ Image accessibility verification failed:',
        error.message
      );
      this.testResults.accessibility = {
        passed: false,
    ...
    try {
      ...
      const blogImageTest = {
        ...
        allChecksPass:
          ...
          buildExists &&
          s3Exists &&
          accessibilityResult?.success !== false,
      };
      ...
      if (accessibilityResult) {
        console.log(
          `   CloudFront accessible: ${accessibilityResult.success ? '✅' : '❌'}`
        );
        if (accessibilityResult.success) {
          console.log(
            `   CloudFront MIME type: ${accessibilityResult.contentType}`
          );
        }
      }
  ...
  generateReport() {
    ...
    if (this.testResults.buildVerification.passed !== undefined) {
      ...
      if (this.testResults.buildVerification.sourceCount !== undefined) {
        console.log(
          `   Source images: ${this.testResults.buildVerification.sourceCount}`
        );
        console.log(
          `   Build images: ${this.testResults.buildVerification.buildCount}`
        );
        if (this.testResults.buildVerification.missingCount > 0) {
          console.log(
            `   Missing: ${this.testResults.buildVerification.missingCount}`
          );
        }
      }
    }
    ...
    if (this.testResults.mimeTypes.totalChecked !== undefined) {
      ...
      if (this.testResults.mimeTypes.incorrectCount > 0) {
        console.log(
          `   Incorrect: ${this.testResults.mimeTypes.incorrectCount}`
        );
      }
    }
    ...
    if (this.testResults.accessibility.skipped) {
      ...
      console.log(
        `   Accessible: ${this.testResults.accessibility.successfulCount}`
      );
    ...
    const reportPath = path.join(
      process.cwd(),
      'deployment-image-verification-report.json'
    );
  ...
  async fixDeploymentIssues() {
    ...
    if (fs.existsSync(deployScriptPath)) {
      ...
      if (!deployScript.includes('image/webp')) {
        ...
        console.log(
          '   ✅ Deploy script already handles WebP MIME types correctly'
        );
      }
    }
    ...
    if (fs.existsSync(nextConfigPath)) {
      ...
      if (
        ...
        console.log(
          '   ℹ️  Next.js config should include static export settings'
        );
      } else {
