...
async function purgeCache() {
  try {
    ...
    console.log(`📁 Paths: ${invalidation.Invalidation.InvalidationBatch.Paths.Items.join(', ')}`);
    ...
    if (error.message.includes('aws: command not found')) {
      console.log('💡 AWS CLI not found. Please install AWS CLI or use alternative method.');
      console.log('   Alternative: Use AWS Console to create invalidation manually');
      console.log(`   Distribution ID: ${DISTRIBUTION_ID}`);
      console.log('   Path to invalidate: /*');
    } else if (error.message.includes('credentials')) {
      console.log('💡 AWS credentials not configured. Please run: aws configure');
    } else {
