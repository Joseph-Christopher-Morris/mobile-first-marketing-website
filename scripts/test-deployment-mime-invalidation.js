...
async function testDeploymentConfiguration() {
  ...
  try {
    ...
    for (const imageFile of projectImageFiles) {
      ...
      console.log(`${mimeCorrect && cacheCorrect ? '✅' : '❌'} ${path.basename(imageFile)}`);
    ...
    console.log(`   MIME types correct: ${mimeTestsPassed}/${projectImageFiles.length}`);
    console.log(`   Cache headers correct: ${cacheTestsPassed}/${projectImageFiles.length}`);
    ...
    console.log(`   Images that will be invalidated: ${deployment.invalidationPaths.length}/${projectImageFiles.length}`);
    console.log(`   Expected: ${projectImageFiles.length} (all images should be invalidated)`);
    
    const allImagesWillBeInvalidated = deployment.invalidationPaths.length === projectImageFiles.length;
    console.log(`   Result: ${allImagesWillBeInvalidated ? '✅ All images will be invalidated' : '❌ Some images will not be invalidated'}`);
    ...
    if (imagePaths.length > 20) {
      pathsToInvalidate = pathsToInvalidate.filter(path => 
        !/\/images\/.*\.(webp|jpg|jpeg|png|gif|svg|ico|avif)$/i.test(path)
      );
      pathsToInvalidate.push('/images/*');
      console.log(`   Optimized to: ${pathsToInvalidate.length} paths`);
      console.log(`   Used /images/* wildcard for ${imagePaths.length} image files`);
    }
    
    const optimizationWorked = pathsToInvalidate.includes('/images/*') && pathsToInvalidate.length < allPaths.length;
    console.log(`   Optimization: ${optimizationWorked ? '✅ Working correctly' : '❌ Not working'}`);
    
    // Summary
    console.log('\n📊 Overall Test Results:');
    const allTestsPassed = mimeTestsPassed === projectImageFiles.length && 
                          cacheTestsPassed === projectImageFiles.length &&
                          allImagesWillBeInvalidated &&
                          optimizationWorked;
    
    console.log(`   MIME Types: ${mimeTestsPassed === projectImageFiles.length ? '✅' : '❌'}`);
    console.log(`   Cache Headers: ${cacheTestsPassed === projectImageFiles.length ? '✅' : '❌'}`);
    console.log(`   Image Invalidation: ${allImagesWillBeInvalidated ? '✅' : '❌'}`);
  ...
  testDeploymentConfiguration()
    .then((passed) => {
      if (passed) {
        ...
        console.log('The deployment script is ready for production use with proper:');
